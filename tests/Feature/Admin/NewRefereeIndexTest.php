...
test('new referee index component can render', function () {
    $user = User::factory()->create();

    Livewire::actingAs($user)
        ->test(NewRefereeIndex::class)
        ->assertStatus(200);
});

test('can create a new referee without a photo', function () {
    $user = User::factory()->create();

    Livewire::actingAs($user)
        ->test(NewRefereeIndex::class)
        ->set('name', 'New Ref Name')
        ->set('email', 'newreferee@example.com')
    ...
    expect($referee)->not->toBeNull()
        ->and($referee->name)->toBe('New Ref Name')
        ->and($referee->user->email)->toBe('newreferee@example.com')
        ->and($referee->photo)->toBeNull();
});

test('can create a new referee with a photo', function () {
    Storage::fake('public');
    $user = User::factory()->create();
    ...
    Livewire::actingAs($user)
        ->test(NewRefereeIndex::class)
        ->set('name', 'New Ref Name Photo')
        ->set('email', 'newrefereephoto@example.com')
        ->set('password', 'password123')
        ->set('phone', '0812345678')
        ->set('nik', '1234567890123456')
        ->set('photo', $photo)
...
test('can edit and update a new referee and replace their photo', function () {
    Storage::fake('public');
...
test('deleting a new referee deletes their photo from storage', function () {
    Storage::fake('public');
    $admin = User::factory()->create();
