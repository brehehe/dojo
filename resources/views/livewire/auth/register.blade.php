<div class="bg-white/10 backdrop-blur-xl border border-white/20 rounded-[3rem] p-10 shadow-2xl shadow-black/50">
    ...
    <div class="flex flex-col items-center mb-10 text-center">
        ...
        <p class="text-orange-400 text-[10px] font-black uppercase tracking-[0.3em] mt-2">Registrasi Kontingen
            Smart-Perkemi</p>
    </div>
    ...
    <form wire:submit.prevent="register" class="space-y-4">
        <div>
            <label for="name"
                class="block text-[10px] font-black text-white/50 uppercase tracking-widest mb-2 px-6">Nama Lengkap /
                Jabatan</label>
            ...
            @error('name') <span class="text-red-400 text-[10px] font-bold mt-1 block px-6">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="email" id="email" wire:model.defer="email" placeholder="contoh@kempo.id"
                class="w-full pl-6 pr-8 py-3 bg-slate-950/30 border-2 border-white/10 rounded-full focus:border-orange-500/50 focus:ring-4 focus:ring-orange-500/10 outline-none transition-all text-slate-100 font-semibold placeholder:text-white/20">
            @error('email') <span class="text-red-400 text-[10px] font-bold mt-1 block px-6">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="password" id="password" wire:model.defer="password" placeholder="••••••••"
                class="w-full pl-6 pr-8 py-3 bg-slate-950/30 border-2 border-white/10 rounded-full focus:border-orange-500/50 focus:ring-4 focus:ring-orange-500/10 outline-none transition-all text-slate-100 font-semibold placeholder:text-white/20">
            @error('password') <span class="text-red-400 text-[10px] font-bold mt-1 block px-6">{{ $message }}</span>
            @enderror
        </div>

        <div>
            <label for="password_confirmation"
                class="block text-[10px] font-black text-white/50 uppercase tracking-widest mb-2 px-6">Konfirmasi Kata
                Sandi</label>
    ...
    <div class="mt-8 pt-8 border-t border-white/5 text-center flex flex-col gap-4">
        ...
        <a href="/"
            class="text-[10px] font-black text-white/20 uppercase tracking-widest hover:text-white transition-colors leading-none block">
