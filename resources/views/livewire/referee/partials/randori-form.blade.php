<div class="space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
            <span class="text-[10px] uppercase text-slate-400 font-bold w-20">Hari :</span>
            <span class="text-sm font-black text-slate-800 bg-slate-50 px-3 py-1 rounded-lg flex-1 border border-slate-100">{{ \Carbon\Carbon::parse($assignedSession->date ?? now())->translatedFormat('l') }}</span>
        </div>
        <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
            <span class="text-[10px] uppercase text-slate-400 font-bold w-20">Tanggal :</span>
            <span class="text-sm font-black text-slate-800 bg-slate-50 px-3 py-1 rounded-lg flex-1 border border-slate-100 whitespace-nowrap">{{ \Carbon\Carbon::parse($assignedSession->date ?? now())->translatedFormat('d F Y') }}</span>
        </div>
        <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
            <span class="text-[10px] uppercase text-slate-400 font-bold w-20">Tingkat :</span>
            <span class="text-sm font-black text-slate-800 bg-slate-50 px-3 py-1 rounded-lg flex-1 border border-slate-100">{{ $activeMatch->ageGroup->name ?? '-' }}</span>
        </div>
        <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
            <span class="text-[10px] uppercase text-slate-400 font-bold w-20">Kelas :</span>
            <span class="text-sm font-black text-slate-800 bg-slate-50 px-3 py-1 rounded-lg flex-1 border border-slate-100">{{ $activeMatch->weightGroup->name ?? '-' }}</span>
        </div>
        <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between">
            <span class="text-[10px] uppercase text-slate-400 font-bold w-20">Court :</span>
            <span class="text-sm font-black text-slate-800 bg-slate-50 px-3 py-1 rounded-lg flex-1 border border-slate-100 text-center">{{ $assignedCourt->order ?? '-' }}</span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-rose-50 border border-rose-200 rounded-2xl p-6">
            ...
            <div class="space-y-4">
                <div class="flex items-center">
                    <span class="w-24 text-[11px] font-bold text-rose-900 uppercase">Nama :</span>
                ...
                <div class="flex items-center">
                    <span class="w-24 text-[11px] font-bold text-rose-900 uppercase">Kontingen :</span>
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-2xl p-6">
            <h3 class="text-xl font-black text-blue-700 uppercase tracking-widest mb-6 flex items-center">
                <i class="fas fa-tag mr-2 text-slate-400 -rotate-45"></i> PITA PUTIH (SHIRO)
            </h3>
            <div class="space-y-4">
                <div class="flex items-center">
                    <span class="w-24 text-[11px] font-bold text-blue-900 uppercase">Nama :</span>
                ...
                <div class="flex items-center">
                    <span class="w-24 text-[11px] font-bold text-blue-900 uppercase">Kontingen :</span>
    ...
    <div class="bg-white border text-sm border-slate-200 rounded-2xl overflow-hidden shadow-sm">
        ...
        <table class="w-full text-center border-collapse">
            <thead class="bg-slate-900 text-slate-300 text-[10px]">
                <tr>
            ...
            <tbody class="font-bold text-sm">
                ...
                @foreach($randoriRows as $row)
                    <tr class="border-b border-slate-100 hover:bg-slate-50">
                        <!-- AKA Columns -->
                        <td class="py-4 border-r border-slate-100 text-slate-500">{{ $row['lbl'] }}</td>
                        <td class="py-4 border-r border-slate-100">{{ $row['desc'] }}</td>
                        <td class="py-4 border-r border-slate-100 text-slate-400">{{ $row['val'] }}</td>
