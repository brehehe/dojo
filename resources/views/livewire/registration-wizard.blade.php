        ...
        DB::transaction(function () {
            ...
            foreach ($this->athletes as $athleteData) {
                $athlete = $contingent->athletes()->create([
                    ...
                    'achievement_history' => $athleteData['achievement_history'] ?: null,
                ]);
...
<div class="max-w-4xl mx-auto p-4 sm:p-8 bg-white shadow-2xl rounded-3xl border border-zinc-200 backdrop-blur-xl bg-opacity-90">
    <div class="mb-10 relative">
        <div class="flex justify-between mb-4">
            @foreach([1, 2, 3, 4] as $index)
                <div @class([
                    'w-10 h-10 rounded-full flex items-center justify-center font-bold transition-all duration-500 z-10',
                    'bg-indigo-600 text-white ring-4 ring-indigo-200 scale-110 shadow-lg' => $step === $index,
                    'bg-emerald-500 text-white' => $step > $index,
                    'bg-zinc-100 text-zinc-400' => $step < $index,
                ])>
                    {{ $step > $index ? '✓' : $index }}
                </div>
            @endforeach
        </div>
        <div class="absolute top-5 left-0 w-full h-0.5 bg-zinc-100 -z-0"></div>
    ...
    @if($step === 1)
        <div class="space-y-6 animate-fade-in">
            <div>
                <h2 class="text-3xl font-extrabold text-zinc-900 tracking-tight">Detail Kontingen</h2>
    ...
    @elseif($step === 2)
        <div class="space-y-6 animate-fade-in text-white/90">
            <div class="flex justify-between items-center bg-indigo-50 p-6 rounded-2xl border border-indigo-100">
                <div>
                    <h2 class="text-2xl font-bold text-indigo-900">Daftar Atlet</h2>
                    <p class="text-indigo-600">Tambahkan semua atlet kontingen Anda.</p>
                </div>
            ...
            <div class="space-y-4">
                @foreach($athletes as $index => $athlete)
                    <div wire:key="athlete-{{ $index }}" class="p-6 rounded-2xl border border-zinc-200 bg-zinc-50 relative group transition-all hover:ring-2 hover:ring-indigo-500/20">
                        @if(count($athletes) > 1)
    ...
    @elseif($step === 3)
        <div class="space-y-6 animate-fade-in text-white/90">
            ...
            @foreach($athletes as $index => $athlete)
                <div class="mb-8 p-6 rounded-2xl border border-zinc-200 bg-white">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        @foreach($availableCategories as $category)
                            <label class="flex items-start p-4 rounded-xl border border-zinc-200 hover:border-indigo-500 cursor-pointer group transition-all">
    ...
    @elseif($step === 5)
        <div class="text-center py-20 animate-fade-in text-white/90">
            <div class="w-24 h-24 bg-emerald-100 text-emerald-500 rounded-full flex items-center justify-center mx-auto mb-8 shadow-2xl pulse-animation">
    ...
    @if($step < 5)
        <div class="flex justify-between mt-12 pt-8 border-t border-zinc-100">
            @if($step > 1)
                <button wire:click="prevStep" class="px-8 py-3 rounded-2xl font-bold text-zinc-600 hover:bg-zinc-100 transition-all flex items-center gap-2">
            ...
            @if($step < 4)
                <button wire:click="nextStep" class="bg-zinc-900 text-white px-10 py-4 rounded-2xl font-black shadow-xl hover:scale-105 active:scale-95 transition-all flex items-center gap-2">
                    Lanjutkan
