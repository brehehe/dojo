<div class="space-y-6 animate-in fade-in duration-700">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div class="space-y-1">
            ...
            <div class="flex items-center gap-2">
                ...
                <p class="text-[11px] text-slate-500 font-bold uppercase tracking-wider italic">{{ $contingent->name }} — {{ $contingent->kab_kota }}</p>
            </div>
        </div>
        <div class="w-full md:w-auto">
            <a href="/piala_walikotasby2026" class="w-full md:w-auto group bg-gradient-to-br from-orange-500 to-orange-700 text-white px-6 py-3 rounded-2xl font-bold shadow-lg shadow-orange-600/20 transition-all flex items-center justify-center gap-3 active:scale-95">
                ...
                <span class="uppercase text-[11px] tracking-widest">Buka Registrasi Baru</span>
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex items-center gap-5 group hover:shadow-xl transition-all duration-300">
            ...
            <div>
                <span class="text-[10px] font-black uppercase tracking-widest text-slate-400">Total Pendaftaran</span>
        ...
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex items-center gap-5 group hover:shadow-xl transition-all duration-300">
            ...
            <div>
                <span class="text-[10px] font-black uppercase tracking-widest text-slate-400">Total Atlet</span>
        ...
        <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex items-center gap-5 group hover:shadow-xl transition-all duration-300">
            ...
            <div>
                <span class="text-[10px] font-black uppercase tracking-widest text-slate-400">Total Official</span>
    ...
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
        <div class="px-8 py-6 border-b border-slate-50 flex justify-between items-center">
            ...
            <span class="px-3 py-1 bg-slate-100 rounded-full text-[10px] font-black text-slate-400 tracking-tighter uppercase">Last Updates: {{ now()->format('d M Y') }}</span>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50">
                        <th class="py-4 px-8 text-slate-400 text-[9px] font-black uppercase tracking-[0.2em]">Info Pendaftaran</th>
                        <th class="py-4 px-8 text-slate-400 text-[9px] font-black uppercase tracking-[0.2em] text-center">Partisipan</th>
                        <th class="py-4 px-8 text-slate-400 text-[9px] font-black uppercase tracking-[0.2em] text-center">Status</th>
                        <th class="py-4 px-8 text-slate-400 text-[9px] font-black uppercase tracking-[0.2em] text-right">Biaya</th>
                    </tr>
                </thead>
                    ...
                    @forelse($registrations as $reg)
                        <tr class="group hover:bg-slate-50/50 transition-colors cursor-pointer">
                            <td class="py-5 px-8">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[10px] text-slate-400 font-semibold">{{ $reg->created_at->format('d M Y, H:i') }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="py-5 px-8">
                                <div class="flex flex-col items-center gap-1">
                                    <span class="text-xs font-black text-slate-700">{{ $reg->athletes_count }} <span class="text-[9px] text-slate-400 lowercase">atlets</span></span>
                                    <span class="text-xs font-black text-slate-700">{{ $reg->officials_count }} <span class="text-[9px] text-slate-400 lowercase">officials</span></span>
                                </div>
                            </td>
                            <td class="py-5 px-8 text-center text-center">
                                ...
                                @endphp
                                <span class="px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border {{ $statusClasses }}">
                                    {{ $reg->status }}
                                </span>
                            </td>
                            <td class="py-5 px-8 text-right">
                                <p class="text-sm font-black text-slate-800 tracking-tighter">Rp {{ number_format($reg->final_amount, 0, ',', '.') }}</p>
                                <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{{ $reg->payment_method }}</p>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="py-20 text-center">
                                <div class="flex flex-col items-center gap-4">
                                    ...
                                    <p class="font-black text-slate-400 uppercase tracking-widest text-[10px]">Belum ada pendaftaran turnamen</p>
