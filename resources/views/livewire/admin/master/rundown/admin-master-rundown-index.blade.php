    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div class="space-y-1">
            ...
            <div class="flex items-center gap-2">
                ...
                <p class="text-[15px] text-slate-900 font-bold uppercase tracking-wider">Master Data Rundown</p>
            </div>
        </div>
        <div class="w-full md:w-auto">
            <button wire:click="showCreateModal"
                ...
                <span class="uppercase text-[15px] tracking-[0.2em]">Tambah Rundown</span>
            </button>
        </div>
    </div>
                    ...
                    @forelse($rundowns as $rundown)
                        <tr class="{{ $loop->even ? 'bg-slate-100' : 'bg-white' }} hover:bg-slate-50 transition-colors group">
                            <td class="py-4 px-6 border-r border-slate-200">
                                <span
                                    class="font-bold text-slate-800 group-hover:text-orange-600 transition-colors text-[15px]">{{ $rundown->name }}</span>
                            </td>
                            <td class="py-4 px-6 border-r border-slate-200">
                                <span
                                    class="font-bold text-slate-800 group-hover:text-orange-600 transition-colors text-[15px]">{{ $rundown->description }}</span>
                            </td>
                             <td class="py-4 px-6 border-r border-slate-200">
                                <span class="px-2.5 py-1 rounded-lg text-[15px] font-black uppercase tracking-wider {{ $rundown->type === 'pertandingan' ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-slate-900' }}">
                                    {{ $rundown->type ?? 'pertandingan' }}
                                </span>
                            </td>
                            <td class="py-4 px-6 border-r border-slate-200">
                                <span class="font-bold text-slate-900 text-[15px]">{{ $rundown->date ? \Carbon\Carbon::parse($rundown->date)->format('d M Y, H:i') : '-' }}</span>
                            </td>
        ...
        <x-modal wire:model.live="showingRundownModal"
            ...
            <div class="space-y-6">
                <div class="space-y-1.5">
                    <label class="text-[15px] font-black uppercase tracking-widest text-slate-800 ml-1">Nama Rundown</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1.5">
                        <label class="text-[15px] font-black uppercase tracking-widest text-slate-800 ml-1">Tipe</label>
                        <select wire:model="type" class="w-full bg-slate-50 border-slate-200 rounded-xl text-[15px] font-bold focus:ring-orange-500/20 focus:border-orange-500">
                            <option value="pertandingan">Pertandingan</option>
                            <option value="seremonial">Seremonial</option>
                        </select>
                        @error('type') <p class="text-[15px] text-red-500 mt-1.5 ml-1 font-bold italic">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="space-y-1.5">
                        <label class="text-[15px] font-black uppercase tracking-widest text-slate-800 ml-1">Tanggal & Waktu</label>
                        <x-input wire:model="date" type="datetime-local" />
                        @error('date') <p class="text-[15px] text-red-500 mt-1.5 ml-1 font-bold italic">{{ $message }}</p>
                        @enderror
                    </div>
                </div>
                <div class="space-y-1.5">
                    <label class="text-[15px] font-black uppercase tracking-widest text-slate-800 ml-1">Deskripsi</label>
                    <x-input wire:model="description" type="text" placeholder="Detail keterangan acara..." />
                    @error('description') <p class="text-[15px] text-red-500 mt-1.5 ml-1 font-bold italic">{{ $message }}</p>
                    @enderror
                </div>
            </div>
