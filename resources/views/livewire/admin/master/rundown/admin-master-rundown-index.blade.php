<div class="space-y-4 animate-in fade-in duration-700">
                ...
                <tbody class="text-slate-700 font-medium divide-y divide-slate-50">
                    @forelse($rundowns as $rundown)
                        <tr class="group hover:bg-slate-50/80 transition-all duration-300">
                            ...
                            <td class="py-2 px-4">
                                <span
                                    class="font-bold text-slate-800 group-hover:text-orange-600 transition-colors text-[13px]">{{ $rundown->start_time }}</span>
                            </td>
                            <td class="py-2 px-4">
                                <span
                                    class="font-bold text-slate-800 group-hover:text-orange-600 transition-colors text-[13px]">{{ $rundown->end_time }}</span>
                            </td>
    ...
    @if($showingRundownModal)
        <x-modal wire:model.live="showingRundownModal"
            ...
            <form wire:submit="saveRundown" class="space-y-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <x-input wire:model="description" type="text" placeholder="Contoh: Jurus 1" />
                    @error('description') <p class="text-[9px] text-red-500 mt-1.5 ml-1 font-bold italic">{{ $message }}</p>
                    @enderror
                </div>
                <div class="space-y-1.5">
                    <label class="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">Tanggal Mulai</label>
                    <x-input wire:model="start_time" type="datetime-local" placeholder="Contoh: 2022-01-01" />
                    @error('start_time') <p class="text-[9px] text-red-500 mt-1.5 ml-1 font-bold italic">{{ $message }}</p>
                    @enderror
                </div>
                <div class="space-y-1.5">
                    <label class="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">Tanggal Selesai</label>
                    <x-input wire:model="end_time" type="datetime-local" placeholder="Contoh: 2022-01-01" />
                    @error('end_time') <p class="text-[9px] text-red-500 mt-1.5 ml-1 font-bold italic">{{ $message }}</p>
                    @enderror
                </div>
            </form>
