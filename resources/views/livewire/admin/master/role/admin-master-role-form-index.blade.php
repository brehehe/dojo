        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100 space-y-6">
                <div class="flex items-center gap-3 pb-2 border-b border-slate-50">
                    ...
                    <h3 class="text-[15px] font-black uppercase tracking-widest text-slate-800">Nama Role</h3>
                </div>

                <div class="space-y-1.5">
                    <label class="text-[15px] font-black uppercase tracking-widest text-slate-800 ml-1">Identitas Jabatan</label>
            ...
            <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100 space-y-8">
                <div class="flex items-center justify-between pb-2 border-b border-slate-50">
                    <div class="flex items-center gap-3">
                        <div class="h-5 w-1.5 bg-orange-600 rounded-full"></div>
                        <h3 class="text-[15px] font-black uppercase tracking-widest text-slate-800">Capabilities & Permissions</h3>
                    </div>
                    <p class="text-[15px] text-slate-800 font-bold uppercase tracking-widest italic">Pilih fitur yang dapat diakses</p>
                </div>
                ...
                @if($permissionGroups->count() > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        @foreach($permissionGroups as $group => $permissions)
                            <div class="space-y-4">
                                <div class="flex items-center gap-2">
                                    <span class="px-2 py-0.5 rounded-md bg-orange-50 text-orange-600 text-[15px] font-black uppercase tracking-widest border border-orange-100">{{ $group }}</span>
                                    ...
                                    @foreach($permissions as $permission)
                                        <label class="group flex items-center gap-3 p-3 rounded-xl border border-slate-50 hover:border-orange-100 hover:bg-orange-50/30 cursor-pointer transition-all duration-300">
                                            ...
                                            <div class="flex flex-col">
                                                <span class="text-[15px] font-black text-black group-hover:text-orange-700 transition-colors capitalize">{{ str_replace('_', ' ', str_replace($group . '.', '', $permission->name)) }}</span>
                                                <span class="text-[15px] text-slate-800 font-bold uppercase tracking-widest italic leading-none">{{ $permission->name }}</span>
                                            </div>
                                        </label>
                                    @endforeach
                                </div>
                ...
                @else
                    <div class="py-20 text-center space-y-4">
                        ...
                        <div class="space-y-1">
                            <p class="text-[15px] font-black uppercase tracking-widest text-slate-800 italic">Belum ada permission terdaftar di sistem.</p>
                            <p class="text-[15px] text-slate-800 font-medium italic">Gunakan seeder atau command line untuk membuat permission baru.</p>
                        </div>
                    </div>
                @endif
            </div>
