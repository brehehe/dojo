<div class="space-y-4 animate-in fade-in duration-700">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="bg-gradient-to-br from-orange-500 to-orange-700 rounded-lg p-4 shadow-lg flex flex-col justify-center border border-white/10 relative overflow-hidden group">
            ...
            <span class="text-[9px] font-black uppercase tracking-[0.2em] text-white/70 mb-0.5 relative z-10">Total Grade</span>
            <div class="flex items-baseline gap-2 relative z-10">
                <span class="text-xl font-black text-white leading-none tracking-tighter">{{ $kyuLevels->total() }}</span>
                <span class="text-[9px] font-black text-white/70 uppercase tracking-widest">Tingkat</span>
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse min-w-[400px]">
                <thead>
                    <tr class="bg-slate-50/50">
                        <th class="py-3 px-6 text-slate-400 text-[9px] font-black uppercase tracking-[0.2em] border-b border-slate-100">Nama Tingkatan</th>
                ...
                <tbody class="text-slate-700 font-medium divide-y divide-slate-50">
                    @forelse($kyuLevels as $kyuLevel)
                        <tr class="group hover:bg-slate-50/80 transition-all duration-300">
                            <td class="py-3 px-6">
                                <span class="font-bold text-slate-800 group-hover:text-orange-600 transition-colors text-[13px]">{{ $kyuLevel->name }}</span>
                            </td>
                            <td class="py-3 px-6 text-right">
                                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
                                    ...
                                    <button type="button" onclick="Swal.fire({
                                                ...
                                                text: 'Data {{ $kyuLevel->name }} akan dihapus permanen!',
                                                icon: 'warning',
                            ...
                            <td colspan="2" class="py-20 text-center">
                                <div class="flex flex-col items-center gap-4">
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <p class="text-slate-400 text-[10px] font-medium italic">Belum ada tingkatan yang terdaftar.</p>
                                    </div>
                                </div>
                            </td>
                        </tr>
        ...
        <div class="fixed inset-0 z-[200] overflow-y-auto" role="dialog" aria-modal="true" x-data x-init="$el.focus()">
            <div class="flex items-center justify-center min-h-screen p-4">
                <div class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm transition-opacity"
                    wire:click="$set('showingkyuLevelModal', false)"></div>
                ...
                <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border border-white/20 animate-in zoom-in-95 duration-200">
                    <div class="bg-gradient-to-br from-slate-800 to-slate-900 p-8 text-white relative">
                        <button wire:click="$set('showingkyuLevelModal', false)" class="absolute top-6 right-6 text-white/50 hover:text-white transition-colors">
                            <i class="fas fa-times"></i>
                        </button>
                        <div class="space-y-1">
                            <span class="text-[9px] font-black uppercase tracking-[0.3em] text-orange-400">Parameter Master</span>
                            <h3 class="text-xl font-black tracking-tight">
                                {{ $kyuLevelIdBeingEdited ? 'Update Tingkatan' : 'Tingkatan Baru' }}
                            </h3>
                        </div>
                    </div>
                    ...
                    <div class="p-8">
                        <form wire:submit="saveKyuLevel" class="space-y-6">
                            <div class="space-y-1.5">
                                <label class="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">Nama Tingkatan (Kyu/Dan)</label>
                                <input wire:model="name" type="text" placeholder="Contoh: Kyu 6 - Putih"
