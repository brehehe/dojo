<div class="space-y-4 animate-in fade-in duration-700">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div class="space-y-1">
            ...
            <div class="flex items-center gap-2">
                ...
                <p class="text-[15px] text-slate-900 font-bold uppercase tracking-wider">Master Data Session Time</p>
            </div>
        </div>
        <div class="w-full md:w-auto">
            <button wire:click="showCreateModal"
                ...
                <span class="uppercase text-[15px] tracking-[0.2em]">Tambah Session Time</span>
            </button>
        </div>
    </div>
    ...
    <x-table>
        <x-slot:header>
            <x-table.th>Nama Session Time</x-table.th>
            <x-table.th>Jam Mulai</x-table.th>
            <x-table.th>Jam Selesai</x-table.th>
        ...
        @forelse($sessionTimes as $sessionTime)
            <x-table.tr>
                <x-table.td>
                    <span class="font-bold text-slate-800 group-hover:text-orange-600 transition-colors text-[15px]">{{ $sessionTime->name }}</span>
                </x-table.td>
                <x-table.td>
                    <span class="font-bold text-slate-800 group-hover:text-orange-600 transition-colors text-[15px]">{{ Carbon\Carbon::parse($sessionTime->start_time)->format('H:i') }}</span>
                </x-table.td>
                <x-table.td>
                    <span class="font-bold text-slate-800 group-hover:text-orange-600 transition-colors text-[15px]">{{ Carbon\Carbon::parse($sessionTime->end_time)->format('H:i') }}</span>
                </x-table.td>
            ...
            <x-table.tr>
                <x-table.td colspan="4" class="py-20 text-center">
                    <div class="flex flex-col items-center gap-4">
                        ...
                        <div class="space-y-1">
                            <p class="font-black text-slate-800 uppercase tracking-widest text-[15px]">Data Session Time Kosong</p>
                        </div>
                    </div>
                </x-table.td>
            </x-table.tr>
        ...
        <x-modal wire:model.live="showingSessionTimeModal"
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1.5">
                        <label class="text-[15px] font-black uppercase tracking-widest text-slate-800 ml-1">Jam
                            Mulai</label>
                        <x-input wire:model="start_time" type="time" />
                        @error('start_time') <p class="text-[15px] text-red-500 mt-1.5 ml-1 font-bold italic">{{
                            $message }}</p>
                        @enderror
                    </div>
                    <div class="space-y-1.5">
                        ...
                        @error('end_time') <p class="text-[15px] text-red-500 mt-1.5 ml-1 font-bold italic">{{
                            $message }}</p>
                        @enderror
                    </div>
                </div>
