<div class="space-y-4 animate-in fade-in duration-700">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div class="space-y-1">
            ...
            <div class="flex items-center gap-2">
                ...
                <p class="text-[15px] text-slate-900 font-bold uppercase tracking-wider">Master Data Kelompok Berat Badan</p>
            </div>
        </div>
        <div class="w-full md:w-auto">
            <button wire:click="showCreateModal"
                ...
                <span class="uppercase text-[15px] tracking-[0.2em]">Tambah Kelompok Berat Badan</span>
            </button>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div
            ...
            <span class="text-[15px] font-black uppercase tracking-[0.2em] text-white/70 mb-0.5 relative z-10">Total Kelompok Berat Badan</span>
            <div class="flex items-baseline gap-2 relative z-10">
                <span class="text-xl font-black text-white leading-none tracking-tighter">{{ $weightGroups->total() }}</span>
                <span class="text-[15px] font-black text-white/70 uppercase tracking-widest">Weight Groups</span>
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse border border-slate-200 rounded-xl overflow-hidden">
                <thead class="bg-slate-800 text-white">
                    <tr class="bg-slate-800">
                        <th class="px-4 py-3 text-[15px] font-black uppercase tracking-widest border border-slate-700 whitespace-nowrap">Nama Kelompok Berat Badan</th>
                    ...
                    @forelse($weightGroups as $weightGroup)
                        <tr class="{{ $loop->even ? 'bg-slate-100' : 'bg-white' }} hover:bg-slate-50 transition-colors group">
                            <td class="py-4 px-6 border-r border-slate-200">
                                <span class="font-bold text-slate-800 group-hover:text-orange-600 transition-colors text-[15px]">{{ $weightGroup->name }}</span>
                            </td>
        ...
        <x-modal wire:model.live="showingWeightGroupModal" title="{{ $weightGroupIdBeingEdited ? 'Update Kelompok Berat Badan' : 'Kelompok Berat Badan Baru' }}" maxWidth="md">
            <form wire:submit="saveWeightGroup" class="space-y-6">
                <div class="space-y-1.5">
                    <label class="text-[15px] font-black uppercase tracking-widest text-slate-800 ml-1">Nama Kelompok Berat Badan</label>
