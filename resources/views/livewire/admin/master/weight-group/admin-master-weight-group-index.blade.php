    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div class="space-y-1">
            ...
            <div class="flex items-center gap-2">
                ...
                <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Master Data Kelompok Berat
                    Badan</p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div
            ...
            <span class="text-[9px] font-black uppercase tracking-[0.2em] text-white/70 mb-0.5 relative z-10">Total
                Kelompok Berat Badan</span>
            <div class="flex items-baseline gap-2 relative z-10">
                <span
                    class="text-xl font-black text-white leading-none tracking-tighter">{{ $weightGroups->total() }}</span>
                <span class="text-[9px] font-black text-white/70 uppercase tracking-widest">Kelompok Berat Badan</span>
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse min-w-[400px]">
                <thead>
                    <tr class="bg-slate-50/50">
                        <th
                            class="py-3 px-6 text-slate-400 text-[9px] font-black uppercase tracking-[0.2em] border-b border-slate-100">
                            Nama Kelompok Berat Badan</th>
                        <th
                            class="py-3 px-6 text-slate-400 text-[9px] font-black uppercase tracking-[0.2em] border-b border-slate-100 text-right">
                            Aksi</th>
                    </tr>
                </thead>
                <tbody class="text-slate-700 font-medium divide-y divide-slate-50">
                    @forelse($weightGroups as $weightGroup)
                        <tr class="group hover:bg-slate-50/80 transition-all duration-300">
                            <td class="py-3 px-6">
                                <span
                                    class="font-bold text-slate-800 group-hover:text-orange-600 transition-colors text-[13px]">{{ $weightGroup->name }}</span>
                            </td>
                                ...
                                <div class="flex flex-col items-center gap-4">
                                    ...
                                    <div class="space-y-1">
                                        <p class="font-black text-slate-400 uppercase tracking-widest text-[10px]">Data
                                            Kosong</p>
                                        <p class="text-slate-400 text-[10px] font-medium italic">Belum ada kelompok umur
                                            yang terdaftar.</p>
                                    </div>
                                </div>
                            </td>
        ...
        <div class="fixed inset-0 z-[200] overflow-y-auto" role="dialog" aria-modal="true" x-data x-init="$el.focus()">
            <div class="flex items-center justify-center min-h-screen p-4">
                <div class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm transition-opacity"
                    wire:click="$set('showingweightGroupModal', false)"></div>

                <div
                    class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border border-white/20 animate-in zoom-in-95 duration-200">
                    <div class="bg-gradient-to-br from-slate-800 to-slate-900 p-8 text-white relative">
                        ...
                        <div class="space-y-1">
                            <span class="text-[9px] font-black uppercase tracking-[0.3em] text-orange-400">Kelompok Berat
                                Badan</span>
                            <h3 class="text-xl font-black tracking-tight">
                                {{ $weightGroupIdBeingEdited ? 'Update Kelompok Berat Badan' : 'Kelompok Berat Badan Baru' }}
                            </h3>
                        </div>
                    </div>
                    ...
                    <div class="p-8">
                        <form wire:submit="saveWeightGroup" class="space-y-6">
                            <div class="space-y-1.5">
                                ...
                                @error('name') <p class="text-[9px] text-red-500 mt-1.5 ml-1 font-bold italic">
                                    {{ $message }}
                                </p> @enderror
                            </div>
