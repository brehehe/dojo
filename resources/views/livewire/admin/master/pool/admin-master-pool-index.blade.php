    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div class="space-y-1">
            ...
            <div class="flex items-center gap-2">
                ...
                <p class="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Master Data Pool</p>
            </div>
        </div>
        <div class="w-full md:w-auto">
            <button wire:click="showCreateModal"
                ...
                <span class="uppercase text-[9px] tracking-[0.2em]">Tambah Pool</span>
            </button>
        </div>
    </div>
                ...
                <tbody class="text-slate-700 font-medium divide-y divide-slate-50">
                    @forelse($pools as $pool)
                        <tr class="group hover:bg-slate-50/80 transition-all duration-300">
                            <td class="py-2 px-4">
                                <span
                                    class="font-bold text-slate-800 group-hover:text-orange-600 transition-colors text-[13px]">{{ $pool->name }}</span>
                            </td>
        ...
        <x-modal wire:model.live="showingPoolModal" title="{{ $poolIdBeingEdited ? 'Update Pool' : 'Pool Baru' }}">
            <form wire:submit="savePool" class="space-y-6">
                <div class="space-y-1.5">
                    <label class="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-1">Nama Pool</label>
