<div>
    ...
    <style>
    /* ══════════════════════════════════════════════════════
       PAGE STYLES — Master Kyu/Dan (Premium Layout)
    ══════════════════════════════════════════════════════ */
    ...
    <div class="prem-page-a">
        ...
        <div class="page-hdr-a">
            <div>
                <h2>Master Kyu / Dan</h2>
                <p>Kelola tingkatan sabuk dan tingkat kemahiran kenshi</p>
            </div>
            <button wire:click="showCreateModal" class="btn-prem-add">
                <i class="fa-solid fa-medal"></i> Tambah Kyu/Dan
            </button>
        </div>
        ...
        <div class="stats-grid-a">
            <div class="stat-card-a gold">
                ...
                <div class="st-val">{{ $kyuLevels->total() }}</div>
                <div class="st-lbl">Total Tingkatan</div>
            </div>
        </div>
        {{-- TABLE --}}
        <div class="table-section-prem">
            <div class="table-toolbar-prem">
                <h3>Daftar Kyu / Dan Level</h3>
            <div class="search-field-prem">
                <i class="fa-solid fa-magnifying-glass"></i>
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari nama tingkatan...">
            </div>
            ...
            <div style="overflow-x:auto;">
                <table class="premium-table">
                    <thead>
                        <tr>
                            <th>Nama Tingkatan (Kyu/Dan)</th>
                            <th>Dibuat Pada</th>
                            <th style="text-align:right;">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($kyuLevels as $level)
                        <tr>
                            <td>
                                <div style="display:flex;align-items:center;gap:12px;">
                                    ...
                                    <span class="group-name">{{ $level->name }}</span>
                                </div>
                            </td>
                            <td>
                                <div style="font-size:12px;font-weight:600;color:var(--ink);">{{ $level->created_at->format('d M Y') }}</div>
                                <div style="font-size:11px;color:var(--smoke);">{{ $level->created_at->diffForHumans() }}</div>
                            </td>
                            <td>
                                <div style="display:flex;gap:5px;justify-content:flex-end;">
                                    <button wire:click="showEditModal({{ $level->id }})" class="act-btn-a edit" title="Edit">
                                        <i class="fa-solid fa-pen"></i>
                                    </button>
                                    <button onclick="confirmDelete({{ $level->id }}, '{{ addslashes($level->name) }}')" class="act-btn-a danger" title="Hapus">
                        ...
                        <tr>
                            <td colspan="3" style="text-align:center;padding:40px 20px;">
                                ...
                                <div style="font-size:12px;color:var(--smoke);">Belum ada data tingkatan yang ditambahkan.</div>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        {{-- MODAL --}}
        @if($showingKyuLevelModal)
        <x-modal wire:model.live="showingKyuLevelModal" maxWidth="md">
            <div style="padding:24px;">
                <div class="modal-hdr">
                    <div class="modal-title">{{ $kyuLevelIdBeingEdited ? 'Edit Tingkatan' : 'Tambah Tingkatan' }}</div>
                </div>
                <form wire:submit="saveKyuLevel">
                    <div class="modal-field">
                        <label>Nama Tingkatan (Kyu/Dan) *</label>
                        <input wire:model="name" type="text" class="modal-input" placeholder="Contoh: Kyu II">
                        @error('name') <span class="modal-err">{{ $message }}</span> @enderror
                    </div>
                    <div class="modal-actions">
                        <button type="button" wire:click="$set('showingKyuLevelModal', false)" class="btn-cancel">Batal</button>
                        <button type="submit" class="btn-save">
                            <span wire:loading.remove wire:target="saveKyuLevel">Simpan Data</span>
                            <span wire:loading wire:target="saveKyuLevel"><i class="fa-solid fa-circle-notch fa-spin"></i> Menyimpan...</span>
                        </button>
                    </div>
                </form>
            </div>
