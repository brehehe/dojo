<div>
    @push('styles')
        <style>
            ...
            .btn-gen.danger { background: var(--red); color: #fff; }
            .btn-gen.danger:hover { background: #a93226; }
            .btn-gen.success { background: #27ae60; color: #fff; }
            .btn-gen.success:hover { background: #229954; }
            ...
            .modal-hdr {
                padding: 20px 24px; border-bottom: 1px solid var(--paper2);
                display: flex; align-items: center; justify-content: space-between;
            }
            .modal-hdr h3 { font-family: 'Cinzel', serif; font-size: 16px; font-weight: 700; margin: 0; color: var(--ink); }
            .modal-close {
                background: var(--paper); border: none; width: 32px; height: 32px; border-radius: 50%;
                display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--smoke);
            }
    ...
    <div class="tm-page" x-data="{ round: 1 }">
        ...
        <div class="tm-hdr">
            <div>
                <div class="tm-badge-title">{{ strtoupper($matchNumber->name) }}</div>
        ...
        @if(isset($availablePools) && $availablePools->count() > 1)
            <div style="display:flex; justify-content:center; gap:8px; margin-bottom:20px;">
                @foreach($availablePools as $p)
                    <button wire:click="setPool({{ $p->id }})" class="btn-gen {{ $selectedPoolId === $p->id ? 'primary' : 'ghost' }}">
                        {{ $p->name }}
                    </button>
                @endforeach
            </div>
        ...
        <div class="info-bar">
            <div class="info-item">
                ...
                <span class="info-value"><i class="far fa-calendar-alt"></i> {{ \Carbon\Carbon::parse($sessionDate)->translatedFormat('l, d M Y') }}</span>
            </div>
            <div class="info-item">
                <span class="info-label">Tingkat / Golongan</span>
                <span class="info-value"><i class="fas fa-layer-group"></i> {{ $matchNumber->ageGroup?->name ?? '-' }}</span>
            </div>
            <div class="info-item">
        ...
        @if(count($tiedIds) > 0)
            <div style="background:rgba(192,57,43,.05); border:1px solid var(--red); border-radius:16px; padding:16px 20px; display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
                <div style="display:flex; align-items:center; gap:16px;">
                    <i class="fas fa-exclamation-triangle" style="font-size:24px; color:var(--red);"></i>
                    <div>
                        <div style="font-size:14px; font-weight:700; color:var(--red); text-transform:uppercase;">Nilai Seri Terdeteksi!</div>
                        <div style="font-size:12px; color:var(--smoke); margin-top:4px;">{{ count($tiedIds) }} peserta memiliki nilai sama di ambang lolos. Lakukan tanding ulang.</div>
                    </div>
                </div>
                <button wire:click="requestTiebreak({{ json_encode($tiedIds) }})" class="btn-gen danger"><i class="fas fa-redo"></i> Tanding Ulang</button>
            </div>
        @elseif($currentRound === 'Penyisihan')
            ...
            @if($hasAllScores && $registrations->count() > 0)
                <div style="background:rgba(39,174,96,.05); border:1px solid #27ae60; border-radius:16px; padding:16px 20px; display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
                    <div style="display:flex; align-items:center; gap:16px;">
                        <i class="fas fa-check-circle" style="font-size:24px; color:#27ae60;"></i>
                        <div>
                            <div style="font-size:14px; font-weight:700; color:#27ae60; text-transform:uppercase;">Penyisihan Selesai!</div>
                            <div style="font-size:12px; color:var(--smoke); margin-top:4px;">Semua peserta dinilai. Siap loloskan ke Final.</div>
                        </div>
                    </div>
                    <button wire:click="advanceToFinal()" class="btn-gen success"><i class="fas fa-arrow-right"></i> Loloskan ke Final</button>
                </div>
            @endif
        @endif
                    ...
                    <tbody>
                        @forelse($registrations as $no => $item)
                            ...
                            <tr style="{{ $isActive ? 'background:#fdfbf7;' : '' }}">
                                ...
                                <td class="text-left">
                                    <div style="font-weight:700; color:var(--ink); font-size:13px; text-transform:uppercase;">
                                        @foreach($item['athletes'] as $athlete){{ $athlete->name }}{{ !$loop->last ? ' & ' : '' }}@endforeach
                                    </div>
                                    <div style="font-size:11px; color:var(--smoke); margin-top:2px;">{{ $item['contingent']?->name ?? '-' }}</div>
                                </td>
                                @foreach([1,2,3,4,5] as $j)
                                    ...
                                    <td>
                                        @if($val > 0)
                                            <span class="score-val {{ $isMinMax ? 'out' : '' }}">{{ number_format($val, 1) }}</span>
                                        @else
                                ...
                                <td>
                                    <div class="score-final">{{ $nilaiAkhir > 0 ? number_format($nilaiAkhir, 1) : '-' }}</div>
                                    @if($denda > 0) <div style="font-size:9px; color:var(--red); font-weight:700; margin-top:2px;">-{{ $denda }} Denda</div> @endif
                                </td>
                                @if($currentRound === 'Final')
                                    <td>{{ isset($item['penyisihan_score']) && $item['penyisihan_score'] ? number_format($item['penyisihan_score']->nilai_akhir, 1) : '-' }}</td>
                                    <td style="font-weight:700; color:#27ae60;">{{ $item['accumulated_score'] > 0 ? number_format($item['accumulated_score'], 1) : '-' }}</td>
                                @endif
                            </tr>
                        @empty
                            <tr>
                                <td colspan="12" style="padding:40px; color:var(--smoke); text-align:center;">Tidak ada data peserta</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        @if($matchNumber->active_registration_id)
            ...
            @endphp
            <div class="tm-card" style="border-color: var(--ink); box-shadow: 0 8px 24px rgba(44, 62, 80, 0.1);" wire:ignore x-data="{
                time: 0,
                running: false,
                countdown: 0,
                lastTickSecond: -1,
                ...
                <div class="tm-card-head" style="background:var(--ink); color:#fff; display:flex; align-items:center; justify-content:space-between;">
                    <h3 style="color:#fff;"><i class="fas fa-stopwatch" style="color:#f1c40f; margin-right:8px;"></i> Live Match Timer &bull; {{ $activeRegItem['athletes'][0]->name ?? 'Peserta' }}</h3>
                    <button @click="window.playBuzzer ? window.playBuzzer('/music/eritnhut1992-buzzer-or-wrong-answer-20582.mp3') : null" class="btn-gen ghost" style="padding:4px 8px; border-color:rgba(255,255,255,0.2); color:#fff; background:transparent;" title="Test Suara">
                        <i class="fas fa-volume-up"></i>
                    </button>
                </div>
                <div class="tm-card-body" style="padding: 24px; text-align: center; background:#fdfbf7;">
                    <div style="font-size:48px; font-weight:900; font-family:'DM Sans', monospace; color:var(--ink); letter-spacing:4px; margin-bottom:16px;">
                    ...
                    <div style="display:flex; gap:12px; justify-content:center;">
                        <button x-show="!running && countdown === 0" @click="start()" class="btn-gen primary" style="padding:12px 24px; font-size:14px;"><i class="fas fa-play" style="margin-right:6px;"></i> Mulai</button>
                        <button x-show="running" @click="pause()" class="btn-gen" style="background:#f39c12; color:#fff; padding:12px 24px; font-size:14px;"><i class="fas fa-pause" style="margin-right:6px;"></i> Jeda</button>
                        <button @click="stop()" class="btn-gen ghost" style="padding:12px; font-size:14px;" title="Stop & Reset"><i class="fas fa-redo-alt"></i></button>
                        <button @click="finish()" class="btn-gen success" style="padding:12px 24px; font-size:14px;"><i class="fas fa-flag-checkered" style="margin-right:6px;"></i> Selesai (Simpan)</button>
                    </div>
                    <div style="margin-top:12px; font-size:11px; color:var(--smoke); font-weight:700; text-transform:uppercase; letter-spacing:0.1em;">
                        ...
                        <div class="queue-card {{ $isActive ? 'active' : '' }}">
                            <div class="queue-hdr">
                                ...
                                <div class="queue-info">
                                    <div class="queue-name">@foreach($item['athletes'] as $athlete){{ $athlete->name }}{{ !$loop->last ? ' & ' : '' }}@endforeach</div>
                                    <div class="queue-cont"><i class="fas fa-shield-alt" style="margin-right:4px;"></i>{{ $item['contingent']?->name }}</div>
                                </div>
                            </div>
                            <div class="queue-actions">
                                @if($isActive)
                                    <button wire:click="callParticipant({{ $item['id'] }})" class="btn-gen ghost" style="flex:1;"><i class="fas fa-redo"></i> Ulang</button>
                                    <button wire:click="dismissParticipant()" class="btn-gen danger" style="flex:1;"><i class="fas fa-times"></i> Tutup</button>
                                @else
                                    <button wire:click="callParticipant({{ $item['id'] }})" class="btn-gen primary" style="width:100%;"><i class="fas fa-bullhorn"></i> Panggil</button>
                                @endif
                            </div>
                            @if($isActive)
                                <div style="margin-top:8px; display:flex; justify-content:center;">
                                    <button wire:click="finishMatch({{ $item['id'] }}, 0)" class="btn-gen success" style="width:100%;"><i class="fas fa-flag-checkered"></i> Selesai (Simpan)</button>
                                </div>
                            @endif
                        </div>
                    @endforeach
        ...
        @php
            $ranked = $registrations->filter(fn($i) => ($i['score']?->nilai_akhir > 0) || ($currentRound === 'Final' && $i['accumulated_score'] > 0))
                ->sortByDesc(fn($i) => $currentRound === 'Penyisihan' ? $i['score']->nilai_akhir : $i['accumulated_score'])->values();
        @endphp
        @if($ranked->count() > 0)
        <div class="tm-card" style="margin-top: 20px;">
            <div class="tm-card-head" style="background:var(--ink); color:#fff; border-bottom:none;">
                <h3 style="color:#fff;"><i class="fas fa-trophy" style="color:#f1c40f; margin-right:8px;"></i> Leaderboard Peringkat</h3>
            </div>
            <div style="overflow-x:auto;">
                <table class="score-table">
                    <thead>
                        ...
                        @foreach($ranked as $rno => $item)
                            ...
                            <tr>
                                <td style="font-size:18px; font-weight:700;">
                                    @if($rankNum==1) 🥇 @elseif($rankNum==2) 🥈 @elseif($rankNum==3) 🥉 @else {{ $rankNum }} @endif
                                </td>
                                <td class="text-left" style="font-weight:700; color:var(--ink); text-transform:uppercase;">
                                    @foreach($item['athletes'] as $athlete){{ $athlete->name }}{{ !$loop->last ? ' & ' : '' }}@endforeach
                                </td>
