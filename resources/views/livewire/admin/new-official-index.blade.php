<div>
    ...
    <style>
    /* ══════════════════════════════════════════════════════
       PAGE STYLES — Master Official (Premium Layout)
    ══════════════════════════════════════════════════════ */
    ...
    .badge { display:inline-block; padding:4px 8px; border-radius:6px; font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:.05em; background:rgba(52,152,219,0.1); color:#3498db; }
    ...
    <div class="prem-page-a">
        ...
        <div class="stats-grid-a">
            <div class="stat-card-a gold">
                <div class="st-icon gold"><i class="fa-solid fa-user-tie"></i></div>
                <div class="st-val">{{ $officials->total() }}</div>
                <div class="st-lbl">Total Official</div>
            </div>
        </div>
        {{-- TABLE --}}
        <div class="table-section-prem">
            <div class="table-toolbar-prem">
                <h3>Daftar Official Tim</h3>
            ...
            <select wire:model.live="filterContingent" class="perpage-select-prem" style="max-width:250px;">
            ...
            <div style="overflow-x:auto;">
                <table class="premium-table">
                    <thead>
                        <tr>
                            <th>Profil Official</th>
                            <th>Kontingen Asal</th>
                            <th>No. Kontak</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <div style="display:flex;align-items:center;gap:12px;">
                                    ...
                                    <div>
                                        <div class="group-name">{{ $off->name }}</div>
                                        <div style="margin-top:4px;"><span class="badge">{{ $off->role }}</span></div>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div style="font-size:13px;font-weight:700;color:var(--ink);">{{ $off->contingent->name ?? '-' }}</div>
                                <div style="font-size:11px;color:var(--smoke);">{{ $off->contingent->kab_kota ?? '-' }}</div>
                            </td>
                            <td>
                                <div style="font-size:13px;font-weight:600;color:var(--ink);"><i class="fa-brands fa-whatsapp" style="color:#2ecc71;"></i> {{ $off->phone ?: '-' }}</div>
                            </td>
                            <td>
                        ...
                        <tr>
                            <td colspan="4" style="text-align:center;padding:40px 20px;">
                                <i class="fa-solid fa-users-slash" style="font-size:32px;color:var(--paper2);margin-bottom:12px;"></i>
                                <div style="font-family:'Cinzel',serif;font-weight:700;color:var(--ink);">Data Kosong</div>
                                <div style="font-size:12px;color:var(--smoke);">Belum ada data official.</div>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
