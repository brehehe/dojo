<div>
    ...
    <style>
    /* ══════════════════════════════════════════════════════
       PAGE STYLES — Master Kelompok Berat (Premium Layout)
    ══════════════════════════════════════════════════════ */
    ...
    <div class="prem-page-a">
        ...
        <div class="page-hdr-a">
            <div>
                <h2>Master Kelompok Berat</h2>
                <p>Kelola kelas berat badan untuk kategori randori</p>
            </div>
            <button wire:click="showCreateModal" class="btn-prem-add">
                <i class="fa-solid fa-weight-scale"></i> Tambah Kelas
            </button>
        </div>
        ...
        <div class="stats-grid-a">
            <div class="stat-card-a gold">
                <div class="st-icon gold"><i class="fa-solid fa-weight-scale"></i></div>
                <div class="st-val">{{ $weightGroups->total() }}</div>
                <div class="st-lbl">Total Kelas Berat</div>
            </div>
        </div>
        {{-- TABLE --}}
        <div class="table-section-prem">
            <div class="table-toolbar-prem">
                <h3>Daftar Kelompok Berat Badan</h3>
            <div class="search-field-prem">
                <i class="fa-solid fa-magnifying-glass"></i>
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari nama kelas berat...">
            </div>
            ...
            <div style="overflow-x:auto;">
                <table class="premium-table">
                    <thead>
                        <tr>
                            <th>Nama Kelas Berat Badan</th>
                            <th>Dibuat Pada</th>
                            <th style="text-align:right;">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($weightGroups as $group)
                        <tr>
                            <td>
                        ...
                        <tr>
                            <td colspan="3" style="text-align:center;padding:40px 20px;">
                                ...
                                <div style="font-size:12px;color:var(--smoke);">Belum ada data kelompok berat badan yang ditambahkan.</div>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
            ...
            <div style="padding:12px 16px;border-top:1px solid var(--paper2);">
                {{ $weightGroups->links('livewire.admin.pagination') }}
            </div>
            @endif
        </div>
        ...
        <x-modal wire:model.live="showingWeightGroupModal" maxWidth="md">
            <div style="padding:24px;">
                <div class="modal-hdr">
                    <div class="modal-title">{{ $weightGroupIdBeingEdited ? 'Edit Kelas Berat' : 'Tambah Kelas Berat' }}</div>
                </div>
                <form wire:submit="saveWeightGroup">
                    <div class="modal-field">
                        <label>Nama Kelas Berat Badan *</label>
                        <input wire:model="name" type="text" class="modal-input" placeholder="Contoh: > 60 Kg">
                        @error('name') <span class="modal-err">{{ $message }}</span> @enderror
                    </div>
                    <div class="modal-actions">
                        <button type="button" wire:click="$set('showingWeightGroupModal', false)" class="btn-cancel">Batal</button>
                        <button type="submit" class="btn-save">
                            <span wire:loading.remove wire:target="saveWeightGroup">Simpan Data</span>
                            <span wire:loading wire:target="saveWeightGroup"><i class="fa-solid fa-circle-notch fa-spin"></i> Menyimpan...</span>
                        </button>
                    </div>
                </form>
            </div>
