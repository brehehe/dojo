<div>
    ...
    <style>
    /* ══════════════════════════════════════════════════════
       PAGE STYLES — Master Teknik & Jurus (Premium Layout)
    ══════════════════════════════════════════════════════ */
    ...
    <div class="prem-page-a">
        ...
        <div class="page-hdr-a">
            <div>
                <h2>Master Teknik & Jurus</h2>
                <p>Kelola klasifikasi teknik dan jurus untuk embu</p>
            </div>
            <button wire:click="showCreateModal" class="btn-prem-add">
                <i class="fa-solid fa-hand-fist"></i> Tambah Jurus
            </button>
        </div>
        ...
        <div class="stats-grid-a">
            <div class="stat-card-a gold">
                <div class="st-icon gold"><i class="fa-solid fa-hand-fist"></i></div>
                <div class="st-val">{{ $techniques->total() }}</div>
                <div class="st-lbl">Total Jurus</div>
            </div>
        </div>
        {{-- TABLE --}}
        <div class="table-section-prem">
            <div class="table-toolbar-prem">
                <h3>Daftar Teknik & Jurus</h3>
            <div class="search-field-prem">
                <i class="fa-solid fa-magnifying-glass"></i>
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari nama teknik / jurus...">
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <div style="display:flex;align-items:center;gap:12px;">
                                    ...
                                    <span class="group-name">{{ $tech->name }}</span>
                                </div>
                            </td>
                            <td>
                                <div style="font-size:12px;font-weight:600;color:var(--ink);">{{ $tech->created_at->format('d M Y') }}</div>
                                <div style="font-size:11px;color:var(--smoke);">{{ $tech->created_at->diffForHumans() }}</div>
                            </td>
                            <td>
                                <div style="display:flex;gap:5px;justify-content:flex-end;">
                                    <button wire:click="showEditModal({{ $tech->id }})" class="act-btn-a edit" title="Edit">
                                        <i class="fa-solid fa-pen"></i>
                                    </button>
                                    <button onclick="confirmDelete({{ $tech->id }}, '{{ addslashes($tech->name) }}')" class="act-btn-a danger" title="Hapus">
                        ...
                        <tr>
                            <td colspan="3" style="text-align:center;padding:40px 20px;">
                                ...
                                <div style="font-size:12px;color:var(--smoke);">Belum ada data teknik yang ditambahkan.</div>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <x-modal wire:model.live="showingTechniqueModal" maxWidth="md">
            <div style="padding:24px;">
                <div class="modal-hdr">
                    <div class="modal-title">{{ $techniqueIdBeingEdited ? 'Edit Teknik' : 'Tambah Teknik' }}</div>
                </div>
                <form wire:submit="saveTechnique">
                    <div class="modal-field">
                        <label>Nama Teknik / Jurus *</label>
                        <input wire:model="name" type="text" class="modal-input" placeholder="Contoh: Kumi Embu Pasangan Campuran">
                        @error('name') <span class="modal-err">{{ $message }}</span> @enderror
                    </div>
                    <div class="modal-actions">
                        <button type="button" wire:click="$set('showingTechniqueModal', false)" class="btn-cancel">Batal</button>
                        <button type="submit" class="btn-save">
                            <span wire:loading.remove wire:target="saveTechnique">Simpan Data</span>
                            <span wire:loading wire:target="saveTechnique"><i class="fa-solid fa-circle-notch fa-spin"></i> Menyimpan...</span>
                        </button>
                    </div>
                </form>
            </div>
