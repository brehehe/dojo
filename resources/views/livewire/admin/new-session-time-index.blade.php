<div>
    ...
    <style>
    /* ══════════════════════════════════════════════════════
       PAGE STYLES — Master Session Time (Premium Layout)
    ══════════════════════════════════════════════════════ */
    ...
    <div class="prem-page-a">
        ...
        <div class="page-hdr-a">
            <div>
                <h2>Master Waktu Sesi (Session Time)</h2>
                <p>Kelola slot waktu untuk pertandingan</p>
            </div>
            <button wire:click="showCreateModal" class="btn-prem-add">
                <i class="fa-solid fa-clock"></i> Tambah Waktu Sesi
            </button>
        </div>
        ...
        <div class="stats-grid-a">
            <div class="stat-card-a gold">
                <div class="st-icon gold"><i class="fa-solid fa-clock"></i></div>
                <div class="st-val">{{ $sessionTimes->total() }}</div>
                <div class="st-lbl">Total Sesi Waktu</div>
            </div>
        </div>
        {{-- TABLE --}}
        <div class="table-section-prem">
            <div class="table-toolbar-prem">
                <h3>Daftar Waktu Sesi</h3>
            <div class="search-field-prem">
                <i class="fa-solid fa-magnifying-glass"></i>
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari nama sesi...">
            </div>
            ...
            <div style="overflow-x:auto;">
                <table class="premium-table">
                    ...
                    <tbody>
                        @forelse($sessionTimes as $session)
                        <tr>
                            <td>
                                <div style="display:flex;align-items:center;gap:12px;">
                                    ...
                                    <span class="group-name">{{ $session->name }}</span>
                                </div>
                            </td>
                            <td>
                                <div style="font-size:14px;font-weight:700;color:var(--ink);font-family:monospace;">
                                    {{ \Carbon\Carbon::parse($session->start_time)->format('H:i') }} <span style="color:var(--smoke);margin:0 4px;">—</span> {{ \Carbon\Carbon::parse($session->end_time)->format('H:i') }}
                                </div>
                            </td>
                            <td>
                                <div style="display:flex;gap:5px;justify-content:flex-end;">
                                    <button wire:click="showEditModal({{ $session->id }})" class="act-btn-a edit" title="Edit">
                                        <i class="fa-solid fa-pen"></i>
                                    </button>
                                    <button onclick="confirmDelete({{ $session->id }}, '{{ addslashes($session->name) }}')" class="act-btn-a danger" title="Hapus">
                        ...
                        <tr>
                            <td colspan="3" style="text-align:center;padding:40px 20px;">
                                ...
                                <div style="font-size:12px;color:var(--smoke);">Belum ada data waktu sesi yang ditambahkan.</div>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
            ...
            <div style="padding:12px 16px;border-top:1px solid var(--paper2);">
                {{ $sessionTimes->links('livewire.admin.pagination') }}
            </div>
            @endif
        </div>
        ...
        <x-modal wire:model.live="showingSessionTimeModal" maxWidth="md">
            <div style="padding:24px;">
                <div class="modal-hdr">
                    <div class="modal-title">{{ $sessionTimeIdBeingEdited ? 'Edit Waktu Sesi' : 'Tambah Waktu Sesi' }}</div>
                </div>
                <form wire:submit="saveSessionTime">
                    <div class="modal-field">
                        <label>Nama Sesi *</label>
                        <input wire:model="name" type="text" class="modal-input" placeholder="Contoh: Sesi Pagi 1">
                    ...
                    <div class="grid-2">
                        <div class="modal-field">
                            ...
                            <input wire:model="start_time" type="time" class="modal-input">
                            @error('start_time') <span class="modal-err">{{ $message }}</span> @enderror
                        </div>
                        <div class="modal-field">
                            <label>Jam Selesai *</label>
                            <input wire:model="end_time" type="time" class="modal-input">
                            @error('end_time') <span class="modal-err">{{ $message }}</span> @enderror
                        </div>
                    </div>

                    <div class="modal-actions">
                        <button type="button" wire:click="$set('showingSessionTimeModal', false)" class="btn-cancel">Batal</button>
                        <button type="submit" class="btn-save">
                            <span wire:loading.remove wire:target="saveSessionTime">Simpan Data</span>
                            <span wire:loading wire:target="saveSessionTime"><i class="fa-solid fa-circle-notch fa-spin"></i> Menyimpan...</span>
                        </button>
                    </div>
                </form>
            </div>
