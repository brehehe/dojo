<div>
    ...
    <style>
    /* ══════════════════════════════════════════════════════
       PAGE STYLES — Master Role (Premium Layout)
    ══════════════════════════════════════════════════════ */
    ...
    <div class="prem-page-a">
        ...
        <div class="stats-grid-a">
            <div class="stat-card-a gold">
                ...
                <div class="st-val">{{ $roles->total() }}</div>
                <div class="st-lbl">Total Role</div>
            </div>
        </div>
        {{-- TABLE --}}
        <div class="table-section-prem">
            <div class="table-toolbar-prem">
                <h3>Daftar Role & Akses</h3>
            <div class="search-field-prem">
                <i class="fa-solid fa-magnifying-glass"></i>
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari nama role...">
            </div>
                ...
                <table class="premium-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div style="font-size:14px;font-weight:700;color:var(--ink);">{{ $role->users_count }} Akun</div>
                            </td>
                            <td>
                                <div style="font-size:12px;font-weight:600;color:var(--ink);">{{ $role->created_at->format('d M Y') }}</div>
                                <div style="font-size:11px;color:var(--smoke);">{{ $role->created_at->diffForHumans() }}</div>
                            </td>
                            <td>
                                <div style="display:flex;gap:5px;justify-content:flex-end;">
                                    ...
                                    <button onclick="confirmDelete({{ $role->id }}, '{{ addslashes($role->name) }}')" class="act-btn-a danger" title="Hapus">
                        ...
                        <tr>
                            <td colspan="4" style="text-align:center;padding:40px 20px;">
                                <i class="fa-solid fa-shield-cat" style="font-size:32px;color:var(--paper2);margin-bottom:12px;"></i>
                                <div style="font-family:'Cinzel',serif;font-weight:700;color:var(--ink);">Data Kosong</div>
                                <div style="font-size:12px;color:var(--smoke);">Belum ada data role yang ditambahkan.</div>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
    ...
    function confirmDelete(id, name) {
        Swal.fire({
            title: 'Hapus Role?',
            html: `Data role <b>${name}</b> akan dihapus permanen.`,
            icon: 'warning',
