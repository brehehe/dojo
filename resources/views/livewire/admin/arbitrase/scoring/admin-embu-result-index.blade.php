<div class="space-y-6 animate-in fade-in duration-500">
        ...
        <div class="flex flex-col md:flex-row md:items-start justify-between gap-4">
            <div>
                <div class="flex items-center gap-2 mb-1">
                    <span class="inline-flex items-center gap-1.5 text-[15px] font-black uppercase tracking-[0.25em] text-orange-600 bg-orange-50 border border-orange-200 px-3 py-1 rounded-full">
                ...
                <p class="text-[15px] text-slate-800 font-semibold uppercase tracking-widest mt-0.5">Penyisihan · Final · Tanding Ulang · Konfirmasi Juara</p>
            </div>
            ...
            <div class="flex flex-col gap-2 min-w-[280px]">
                ...
                <div class="flex flex-wrap gap-1.5">
                    <button wire:click="$set('selectedAgeGroupId', null)"
                            class="px-3 py-1.5 rounded-xl text-[15px] font-black uppercase tracking-widest transition-all border
                                   {{ is_null($selectedAgeGroupId) ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-900 border-slate-200 hover:border-slate-300' }}">
                        Semua
                    </button>
                    @foreach($ageGroups as $ag)
                        <button wire:click="$set('selectedAgeGroupId', {{ $ag->id }})"
                                class="px-3 py-1.5 rounded-xl text-[15px] font-black uppercase tracking-widest transition-all border
                                       {{ $selectedAgeGroupId == $ag->id ? 'bg-orange-600 text-white border-orange-600' : 'bg-white text-slate-900 border-slate-200 hover:border-orange-300 hover:text-orange-600' }}">
                            {{ $ag->name }}
                        </button>
                    @endforeach
                </div>
                ...
                <select wire:model.live="selectedMatchId"
                        class="w-full appearance-none bg-white border border-slate-200 text-black text-[12px] font-bold rounded-2xl px-4 py-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-400/30 transition-all">
            ...
            <div class="bg-gradient-to-br from-amber-400 to-orange-500 rounded-3xl p-6 shadow-xl shadow-amber-500/20">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-[15px] font-black text-white/70 uppercase tracking-widest">Hasil Akhir Dikonfirmasi</p>
                    ...
                    <button wire:click="$set('showChampionModal', true)"
                            class="px-3 py-1.5 bg-white/20 hover:bg-white/30 text-white text-[15px] font-black uppercase tracking-widest rounded-xl transition-all">
                    ...
                    @foreach($champions->take(3) as $champ)
                        ...
                        <div class="flex items-center gap-3 bg-white/15 rounded-2xl px-4 py-3">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @endforeach
                                <p class="text-[15px] text-white/60 font-bold uppercase mt-0.5">{{ $champ->registration?->contingent?->name }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-[14px] font-black text-white tabular-nums">{{ number_format($champ->accumulated_score, 1) }}</p>
                                <p class="text-[15px] text-white/60">Akumulasi</p>
                            </div>
                        </div>
                    @endforeach
                    @if($champions->count() > 3)
                        <p class="text-[15px] text-white/50 text-center font-bold">+{{ $champions->count() - 3 }} peserta lainnya</p>
                    @endif
                </div>
            </div>
        @endif
                ...
                <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        <div class="w-7 h-7 bg-slate-100 rounded-lg flex items-center justify-center">
                            <i class="fas fa-list-ol text-slate-900 text-[15px]"></i>
                        </div>
                        <div>
                            <p class="text-[15px] font-black text-black uppercase tracking-widest">Penyisihan</p>
                            <p class="text-[15px] text-slate-800 font-bold">Nilai terendah = terbaik</p>
                        </div>
                    </div>
                    <div class="flex gap-2">
                        @if(!empty($tiedPenyisihanIds))
                            <button wire:click="openTiebreakModal('Penyisihan', {{ json_encode($tiedPenyisihanIds) }})"
                                    class="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 text-[15px] font-black uppercase tracking-widest rounded-xl transition-all border border-rose-200">
                                <i class="fas fa-equals mr-1"></i> Tanding Ulang ({{ count($tiedPenyisihanIds) }})
                            </button>
                        @endif
                        @if(!$finalExists)
                            <button wire:click="openGenerateFinalModal"
                                    class="px-3 py-1.5 bg-orange-600 hover:bg-orange-700 text-white text-[15px] font-black uppercase tracking-widest rounded-xl transition-all shadow-sm">
                                <i class="fas fa-arrow-right mr-1"></i> Generate Final
                            </button>
                        @else
                            <button wire:click="openGenerateFinalModal"
                                    class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-900 text-[15px] font-black uppercase tracking-widest rounded-xl transition-all">
                ...
                <div class="divide-y divide-slate-50">
                    ...
                    @empty
                        <div class="py-16 text-center">
                            ...
                            <p class="text-[15px] font-black text-slate-300 uppercase tracking-widest">Belum ada nilai Penyisihan</p>
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <p class="text-[15px] font-black text-black uppercase tracking-widest">Final</p>
                            <p class="text-[15px] text-slate-800 font-bold">Akumulasi Penyisihan + Final</p>
                        </div>
                    </div>
                    <div class="flex gap-2">
                ...
                @if(!$finalExists)
                    <div class="py-16 text-center">
                        ...
                        <p class="text-[15px] font-black text-slate-800 uppercase tracking-widest">Babak Final Belum Dibuka</p>
                        <p class="text-[15px] text-slate-300 mt-1">Generate Final dari panel Penyisihan</p>
                    </div>
                @else
                    <div class="divide-y divide-slate-50">
                        @forelse($finalRanking as $idx => $reg)
                            ...
                            <div class="px-5 py-3 flex items-center gap-3 {{ $isTiedFinal ? 'bg-rose-50' : '' }}">
                                ...
                                <div class="text-right shrink-0">
                                    @if($isTiedFinal)
                                        <span class="text-[15px] font-black bg-rose-100 text-rose-600 px-1.5 py-0.5 rounded-md uppercase block mb-1">SERI</span>
                                    @endif
                                    <div class="flex gap-3 items-center justify-end text-[15px] text-slate-800 font-bold">
                        ...
                        @empty
                            <div class="py-16 text-center">
                                ...
                                <p class="text-[15px] font-black text-slate-300 uppercase tracking-widest">Peserta Final belum diinput nilainya</p>
                            </div>
                        @endforelse
                    </div>
                @endif
            ...
            <div class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" wire:click.self="$set('showGenerateFinalModal', false)">
                <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95 duration-200">
                    <div class="flex items-center gap-3 mb-5">
                        ...
                        <div>
                            <p class="text-[15px] font-black text-slate-800 uppercase tracking-widest">Jadwal Babak</p>
                    ...
                    <div class="space-y-3 mb-5">
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <select wire:model="finalCourtId" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-[15px] font-bold focus:outline-none focus:ring-2 focus:ring-orange-400/30">
                            ...
                            <div>
                                <label class="text-[15px] font-black text-slate-800 uppercase tracking-widest block mb-1">Pool</label>
                                <select wire:model="finalPoolId" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-[15px] font-bold focus:outline-none focus:ring-2 focus:ring-orange-400/30">
                                    <option value="">– Pilih –</option>
                                    @foreach($pools as $p)
                            ...
                            <div>
                                <label class="text-[15px] font-black text-slate-800 uppercase tracking-widest block mb-1">Sesi</label>
                                <select wire:model="finalSessionTimeId" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-[15px] font-bold focus:outline-none focus:ring-2 focus:ring-orange-400/30">
                                    <option value="">– Pilih –</option>
                            ...
                            <div>
                                <label class="text-[15px] font-black text-slate-800 uppercase tracking-widest block mb-1">Rundown</label>
                                <select wire:model="finalRundownId" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-[15px] font-bold focus:outline-none focus:ring-2 focus:ring-orange-400/30">
                                    <option value="">– Pilih –</option>
                        ...
                        <div>
                            <label class="text-[15px] font-black text-slate-800 uppercase tracking-widest block mb-1">Tanggal Jadwal</label>
                    ...
                    @if(!empty($tiedPenyisihanIds))
                        <div class="bg-rose-50 border border-rose-200 rounded-2xl px-4 py-3 mb-4">
                            <p class="text-[15px] font-black text-rose-700 uppercase"><i class="fas fa-exclamation-triangle mr-1"></i> Ada nilai seri di batas kuota!</p>
                            <p class="text-[15px] text-rose-600 mt-0.5">{{ count($tiedPenyisihanIds) }} peserta perlu tanding ulang sebelum generate final.</p>
                        </div>
                    @endif

                    <div class="flex gap-3">
                        <button wire:click="$set('showGenerateFinalModal', false)"
                                class="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-900 font-black text-[15px] uppercase tracking-widest rounded-2xl transition-all">Batal</button>
                        <button wire:click="generateFinal" wire:loading.attr="disabled"
                                class="flex-1 py-3 bg-orange-600 hover:bg-orange-700 text-white font-black text-[15px] uppercase tracking-widest rounded-2xl transition-all shadow-lg shadow-orange-600/20 disabled:opacity-60">
            ...
            <div class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" wire:click.self="$set('showTiebreakModal', false)">
                <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95 duration-200">
                    <div class="flex items-center gap-3 mb-5">
                        ...
                        <div>
                            <p class="text-[15px] font-black text-slate-800 uppercase tracking-widest">Jadwal Tanding Ulang</p>
                    ...
                    <div class="space-y-3 mb-5">
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="text-[15px] font-black text-slate-800 uppercase tracking-widest block mb-1">Court</label>
                                <select wire:model="tiebreakCourtId" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-[15px] font-bold focus:outline-none focus:ring-2 focus:ring-rose-400/30">
                                    <option value="">– Pilih –</option>
                                    @foreach($courts as $c)
                                        <option value="{{ $c->id }}">{{ $c->name }}</option>
                                    @endforeach
                            ...
                            <div>
                                <label class="text-[15px] font-black text-slate-800 uppercase tracking-widest block mb-1">Sesi</label>
                                <select wire:model="tiebreakSessionTimeId" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-[15px] font-bold focus:outline-none focus:ring-2 focus:ring-rose-400/30">
                                    <option value="">– Pilih –</option>
                                    @foreach($sessionTimes as $s)
                                        <option value="{{ $s->id }}">{{ $s->name }}</option>
                                    @endforeach
                            ...
                            <div>
                                <label class="text-[15px] font-black text-slate-800 uppercase tracking-widest block mb-1">Rundown</label>
                                <select wire:model="tiebreakRundownId" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-[15px] font-bold focus:outline-none focus:ring-2 focus:ring-rose-400/30">
                                    <option value="">– Pilih –</option>
                                    @foreach($rundowns as $r)
                                        <option value="{{ $r->id }}">{{ $r->name }}</option>
                                    @endforeach
                            ...
                            <div>
                                <label class="text-[15px] font-black text-slate-800 uppercase tracking-widest block mb-1">Tanggal</label>
                                <input type="date" wire:model="tiebreakScheduleDate"
                                       class="w-full border border-slate-200 rounded-xl px-3 py-2 text-[15px] font-bold focus:outline-none focus:ring-2 focus:ring-rose-400/30">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-3">
                        ...
                        <button wire:click="generateTiebreakSchedule" wire:loading.attr="disabled"
                                class="flex-1 py-3 bg-rose-600 hover:bg-rose-700 text-white font-black text-[15px] uppercase tracking-widest rounded-2xl transition-all shadow-lg shadow-rose-600/20 disabled:opacity-60">
            ...
            <div class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" wire:click.self="$set('showChampionModal', false)">
                <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95 duration-200">
                    <div class="flex items-center gap-3 mb-5">
                        ...
                        <div>
                            <p class="text-[15px] font-black text-amber-500 uppercase tracking-widest">Konfirmasi Hasil Akhir</p>
                    ...
                    <div class="bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 mb-4">
                        <p class="text-[15px] font-black text-amber-800 uppercase"><i class="fas fa-info-circle mr-1"></i> Aksi ini akan menyimpan / mengupdate data juara</p>
                        <p class="text-[15px] text-amber-600 mt-0.5">Data juara sebelumnya (jika ada) akan diganti dengan hasil terbaru.</p>
                    </div>

                    @if(!empty($tiedFinalIds))
                        <div class="bg-rose-50 border border-rose-200 rounded-2xl px-4 py-3 mb-4">
                            <p class="text-[15px] font-black text-rose-700 uppercase"><i class="fas fa-exclamation-triangle mr-1"></i> Masih ada nilai seri!</p>
                            <p class="text-[15px] text-rose-600 mt-0.5">{{ count($tiedFinalIds) }} peserta memiliki nilai akumulasi sama. Selesaikan tanding ulang terlebih dahulu.</p>
                        </div>
                    @endif
                    ...
                    <div class="flex gap-3">
                        ...
                        <button wire:click="confirmChampion" wire:loading.attr="disabled"
                                class="flex-1 py-3 bg-amber-500 hover:bg-amber-600 text-white font-black text-[15px] uppercase tracking-widest rounded-2xl transition-all shadow-lg shadow-amber-500/20 disabled:opacity-60">
    ...
    @else
        <div class="flex flex-col items-center justify-center bg-white rounded-3xl border-2 border-dashed border-slate-200 py-24 text-center">
            ...
            <p class="text-[15px] font-black text-slate-800 uppercase tracking-widest">Pilih Nomor Embu</p>
            <p class="text-[15px] text-slate-300 mt-1">Gunakan dropdown di atas untuk memulai</p>
        </div>
    @endif
