@php
    ...
    $schemeCallBtn = match($colorScheme) {
        ...
        default  => 'bg-white border border-slate-200 text-slate-500 hover:bg-indigo-50 hover:border-indigo-300 hover:text-indigo-600',
    };
...
<div class="relative w-64">
    {{-- Match label badge --}}
    <div class="absolute -top-3 left-3 z-10 px-1.5 py-0.5 bg-slate-100 text-slate-400 text-[8px] font-black rounded border border-slate-200 uppercase tracking-wide">
        {{ $matchLabel }}
    </div>

    {{-- LIVE badge --}}
    @if($isActive)
        <div class="absolute -top-3 right-3 z-10 px-1.5 py-0.5 bg-amber-500 text-white text-[8px] font-black rounded uppercase animate-pulse">
    ...
    <div class="bg-white rounded-xl border-2
        ...
        @if(!$hasSomeAthlete)
            ...
            <div class="px-3 py-6 flex flex-col items-center justify-center gap-1">
                ...
                <span class="text-[9px] font-bold text-slate-300 uppercase tracking-wide">Menunggu...</span>
            </div>
        @else
            {{-- Athlete 1 (Merah/AKA) --}}
            <div class="px-3 py-2.5 border-b border-slate-100 flex items-center gap-2
                ...
                <div class="flex-1 min-w-0">
                    @if($a1)
                        <div class="text-[10px] font-black uppercase tracking-tight truncate
                            {{ $isDone && $winnerSlot === 'athlete1' ? 'text-rose-700' : 'text-slate-700' }}">
                            {{ $a1['name'] }}
                        </div>
                        @if($a1['contingent'] ?? null)
                            <div class="text-[8px] text-slate-400 truncate">{{ $a1['contingent'] }}</div>
                        @endif
                    @else
                        <div class="text-[10px] text-slate-300 italic font-bold">TBD</div>
                    @endif
                </div>
                @if($isDone && $winnerSlot === 'athlete1')
                    <span class="text-[8px] font-black bg-rose-500 text-white px-1 py-0.5 rounded flex-shrink-0">W</span>
                @endif
            </div>

            {{-- VS --}}
            <div class="py-1 bg-slate-50 flex items-center justify-center text-[8px] font-black text-slate-300 tracking-widest uppercase">vs</div>
            ...
            <div class="px-3 py-2.5 flex items-center gap-2
                ...
                <div class="flex-1 min-w-0">
                    @if($a2)
                        <div class="text-[10px] font-black uppercase tracking-tight truncate
                            {{ $isDone && $winnerSlot === 'athlete2' ? 'text-indigo-700' : 'text-slate-700' }}">
                            {{ $a2['name'] }}
                        </div>
                        @if($a2['contingent'] ?? null)
                            <div class="text-[8px] text-slate-400 truncate">{{ $a2['contingent'] }}</div>
                        @endif
                    @else
                        <div class="text-[10px] text-slate-300 italic font-bold">TBD</div>
                    @endif
                ...
                @if($isDone && $winnerSlot === 'athlete2')
                    <span class="text-[8px] font-black bg-indigo-500 text-white px-1 py-0.5 rounded flex-shrink-0">W</span>
                @endif
            </div>
            ...
            @if($hasBothAthletes && !$isDone)
                <div class="border-t border-slate-100 bg-slate-50/60 px-3 py-2 flex items-center justify-end gap-1.5">
                    <button
                        ...
                        class="inline-flex items-center gap-1 px-2 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-wide transition-all
                    ...
                    <button
                        ...
                        class="inline-flex items-center gap-1 px-2 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-wide bg-slate-900 text-white hover:bg-orange-600 transition-all shadow-sm"
                        title="Input Skor"
                    >
                        <i class="fas fa-edit text-[8px]"></i> Skor
                    </button>
                </div>
            @elseif($isDone)
                <div class="border-t border-slate-100 bg-slate-50/40 px-3 py-1.5 flex items-center justify-center">
                    <span class="text-[8px] font-black text-emerald-500 uppercase tracking-widest"><i class="fas fa-check mr-1"></i>Selesai</span>
                </div>
            @endif
