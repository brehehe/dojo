    ...
    <div class="flex items-center justify-between bg-slate-800 rounded-2xl p-4 md:p-6 shadow-2xl shrink-0 border border-slate-700 relative z-10 w-full mb-6">
            ...
            <div>
                <h1 class="text-2xl md:text-4xl font-black tracking-tight text-white flex items-center gap-3">
                    ...
                    @if($match && $match->draft_type === 'randori')
                        <span class="px-3 py-1 bg-rose-500/20 text-rose-400 border border-rose-500/30 rounded-xl text-[15px] md:text-lg uppercase tracking-widest">RANDORI</span>
                    @elseif($match && $match->draft_type === 'embu')
                        <span class="px-3 py-1 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-xl text-[15px] md:text-lg uppercase tracking-widest">EMBU</span>
                    @endif
                </h1>
                @if($match)
                    <p class="text-amber-400 font-bold tracking-widest text-[15px] md:text-lg uppercase mt-1">
                        {{ $match->name }}
                        @if($match->ageGroup) <span class="text-slate-800 mx-2">•</span> <span class="text-slate-300">{{ $match->ageGroup->name }}</span> @endif
                    </p>
                @endif
            </div>
        </div>
        ...
        <div class="text-right">
            @if($court)
                ...
                <p class="text-slate-800 font-bold tracking-widest uppercase text-[15px] md:text-[15px] mt-1">Update Otomatis</p>
            @else
        ...
        @if(! $match)
            <div class="absolute inset-0 flex flex-col items-center justify-center">
                <div class="w-32 h-32 md:w-48 md:h-48 bg-slate-800 rounded-full flex items-center justify-center shadow-inner mb-6 border border-slate-700">
                    <i class="fas fa-mug-hot text-5xl md:text-7xl text-slate-900"></i>
                </div>
                <h2 class="text-2xl md:text-4xl font-black text-slate-900 uppercase tracking-widest">BELUM ADA PERTANDINGAN AKTIF</h2>
                <p class="text-slate-900 mt-4 font-bold md:text-xl">Silakan tunggu panggilan selanjutnya.</p>
            </div>
        @else
                ...
                <div class="w-full h-full overflow-auto custom-scrollbar p-6">
                    <table class="w-full text-left border-collapse border border-slate-200 rounded-xl overflow-hidden">
                        <thead class="bg-slate-800 text-white">
                            <tr class="bg-slate-950 text-slate-800">
                                <th class="px-4 py-3 text-[15px] font-black uppercase tracking-widest border border-slate-700 whitespace-nowrap text-center">Peringkat</th>
                                <th class="px-4 py-3 text-[15px] font-black uppercase tracking-widest border border-slate-700 whitespace-nowrap">Kontingen / Atlet</th>
                        ...
                        <tbody class="divide-y divide-slate-200">
                            @foreach($embuRanking as $index => $row)
                                ...
                                <tr class="{{ $loop->even ? 'bg-slate-100' : 'bg-white' }} hover:bg-slate-50 transition-colors group">
                                    {{-- Rank --}}
                                    <td class="px-6 py-5 align-middle text-center border-r border-slate-200">
                                        @if($hasScore)
                                            <div class="inline-flex items-center justify-center w-12 h-12 md:w-16 md:h-16 rounded-xl font-black text-xl md:text-3xl {{ $rank === 1 ? 'bg-amber-500 text-slate-900 shadow-lg shadow-amber-500/40' : ($rank === 2 ? 'bg-slate-300 text-slate-800' : ($rank === 3 ? 'bg-amber-700 text-white' : 'bg-slate-800 text-slate-800 border border-slate-700')) }}">
                                                {{ $rank }}
                                            </div>
                                        @else
                                    ...
                                    {{-- Contingent / Athletes --}}
                                    <td class="px-6 py-5 align-middle border-r border-slate-200">
                                        ...
                                        @if($row['athletes']->isNotEmpty())
                                            <div class="mt-2 text-[15px] md:text-base text-slate-800 font-medium">
                                    ...
                                    <td class="px-6 py-5 align-middle text-center border-r border-slate-200">
                                        ...
                                        @else
                                            <span class="text-[15px] md:text-lg text-slate-900 font-black tracking-widest uppercase py-2">Belum Tampil</span>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
                            @if($embuRanking->isEmpty())
                                <tr>
                                    <td colspan="3" class="px-6 py-12 text-center text-slate-900 font-bold text-lg border-r border-slate-200">Belum ada data peserta.</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
                            ...
                            <div class="flex items-start gap-12 relative z-10">
                                @foreach($ubRounds as $roundIdx => $roundMatches)
                                    <div class="flex flex-col gap-6 justify-center" style="min-width: 280px; min-height: 100%;">
                                        <div class="text-center mb-6 sticky top-0 bg-slate-900/80 py-2 rounded font-black text-[15px] md:text-[15px] text-slate-900 tracking-widest uppercase">
                                ...
                                @if($gfMatch)
                                    <div class="flex flex-col gap-6 justify-center" style="min-width: 320px; margin-left: 2rem;">
                                        <div class="text-center mb-6 sticky top-0 bg-slate-900/80 py-2 rounded font-black text-[15px] md:text-base text-amber-500 tracking-widest uppercase">
                                            Grand Final
                                        </div>
                                        @php
                                            $gfNodeKey = "gf_0_0";
