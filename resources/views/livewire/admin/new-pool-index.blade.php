<div>
    ...
    <style>
    /* ══════════════════════════════════════════════════════
       PAGE STYLES — Master Pool (Premium Layout)
    ══════════════════════════════════════════════════════ */
    ...
    <div class="prem-page-a">
        ...
        <div class="page-hdr-a">
            <div>
                <h2>Master Pool (Bagan)</h2>
                <p>Kelola data pool untuk pembagian bagan pertandingan</p>
            </div>
            <button wire:click="showCreateModal" class="btn-prem-add">
                <i class="fa-solid fa-sitemap"></i> Tambah Pool
            </button>
        </div>
        ...
        <div class="stats-grid-a">
            <div class="stat-card-a gold">
                <div class="st-icon gold"><i class="fa-solid fa-sitemap"></i></div>
                <div class="st-val">{{ $pools->total() }}</div>
                <div class="st-lbl">Total Pool</div>
            </div>
        </div>
        {{-- TABLE --}}
        <div class="table-section-prem">
            <div class="table-toolbar-prem">
                <h3>Daftar Bagan (Pool)</h3>
            <div class="search-field-prem">
                <i class="fa-solid fa-magnifying-glass"></i>
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari nama pool...">
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <div style="display:flex;align-items:center;gap:12px;">
                                    ...
                                    <span class="group-name">{{ $pool->name }}</span>
                                </div>
                            </td>
                            <td>
                                <div style="font-size:12px;font-weight:600;color:var(--ink);">{{ $pool->created_at->format('d M Y') }}</div>
                                <div style="font-size:11px;color:var(--smoke);">{{ $pool->created_at->diffForHumans() }}</div>
                            </td>
                            <td>
                                <div style="display:flex;gap:5px;justify-content:flex-end;">
                                    <button wire:click="showEditModal({{ $pool->id }})" class="act-btn-a edit" title="Edit">
                                        <i class="fa-solid fa-pen"></i>
                                    </button>
                                    <button onclick="confirmDelete({{ $pool->id }}, '{{ addslashes($pool->name) }}')" class="act-btn-a danger" title="Hapus">
                        ...
                        <tr>
                            <td colspan="3" style="text-align:center;padding:40px 20px;">
                                ...
                                <div style="font-size:12px;color:var(--smoke);">Belum ada data pool yang ditambahkan.</div>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        {{-- MODAL --}}
        @if($showingPoolModal)
        <x-modal wire:model.live="showingPoolModal" maxWidth="md">
            <div style="padding:24px;">
                <div class="modal-hdr">
                    <div class="modal-title">{{ $poolIdBeingEdited ? 'Edit Pool' : 'Tambah Pool' }}</div>
                </div>
                <form wire:submit="savePool">
                    <div class="modal-field">
                        <label>Nama Pool *</label>
                        <input wire:model="name" type="text" class="modal-input" placeholder="Contoh: Pool A">
                        @error('name') <span class="modal-err">{{ $message }}</span> @enderror
                    </div>
                    <div class="modal-actions">
                        <button type="button" wire:click="$set('showingPoolModal', false)" class="btn-cancel">Batal</button>
                        <button type="submit" class="btn-save">
                            <span wire:loading.remove wire:target="savePool">Simpan Data</span>
                            <span wire:loading wire:target="savePool"><i class="fa-solid fa-circle-notch fa-spin"></i> Menyimpan...</span>
                        </button>
                    </div>
                </form>
            </div>
