@push('styles')
    <style>
        ...
        .bracket-hdr.lb { background: rgba(211, 84, 0, .05); }
        .bracket-hdr.gf { background: rgba(243, 156, 18, .05); }
        ...
        .b-slot+.b-slot { border-top: 1px solid var(--paper2, #e0dcd3); }
        ...
        .official-label i { color: var(--red); font-size: 13px; }
        .official-val {
            font-size: 15px;
            font-weight: 700;
            color: var(--ink);
        ...
        .b-slot-color.red { background: var(--red, #c0392b); }
        .b-slot-color.blue { background: #2980b9; }

        .b-slot-info { flex: 1; min-width: 0; }
        .b-slot-name { font-size: 12.5px; font-weight: 700; color: var(--ink, #2c3e50); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .b-slot-cont { font-size: 10.5px; color: var(--smoke, #7f8c8d); font-weight: 600; text-transform: uppercase; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .b-slot-empty { font-size: 12px; color: var(--smoke, #7f8c8d); font-style: italic; }
        ...
        .b-action-btn:hover { background: #fff; color: var(--red); }

        /* BUTTONS */
        .btn-gen {
            display: inline-flex; align-items: center; justify-content: center; gap: 6px;
            padding: 10px 16px; border-radius: 8px; border: none; font-size: 12px; font-weight: 700;
            cursor: pointer; font-family: 'DM Sans', sans-serif; transition: all .15s;
            text-transform: uppercase; letter-spacing: 0.05em;
        }
        .btn-gen.primary { background: var(--ink, #2c3e50); color: #fff; }
        .btn-gen.primary:hover { background: #1a252f; transform: translateY(-1px); }
        .btn-gen.danger { background: var(--red, #c0392b); color: #fff; }
        .btn-gen.danger:hover { background: #a93226; }
        .btn-gen.ghost { background: #fff; color: var(--ink); border: 1px solid var(--paper2); }
        .btn-gen.ghost:hover { border-color: var(--ink); }

        /* CARDS */
        .tm-card { background: #fff; border: 1px solid var(--paper2); border-radius: 16px; overflow: hidden; margin-bottom: 24px; }
        .tm-card-head { padding: 16px 20px; border-bottom: 1px solid var(--paper2); background: #fdfbf7; display: flex; align-items: center; justify-content: space-between; }
        .tm-card-head h3 { font-family: 'Cinzel', serif; font-size: 15px; font-weight: 700; margin: 0; color: var(--ink); }
        .tm-card-body { padding: 20px; }
        
        .score-table { width: 100%; border-collapse: collapse; font-size: 12px; }
        .score-table th { padding: 12px; background: #fdfbf7; font-size: 10px; color: var(--smoke); font-weight: 700; text-transform: uppercase; border: 1px solid var(--paper2); text-align: center; }
        .score-table td { padding: 12px; border: 1px solid var(--paper2); vertical-align: middle; text-align: center; }
        
        .score-btn { width: 24px; height: 24px; border-radius: 6px; border: 1px solid var(--paper2); background: #fff; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; }
        .score-btn:hover { background: var(--paper); border-color: var(--ink); }
...
<div class="tm-page">
    ...
    <div class="tm-hdr">
        <div>
            <div class="tm-badge-title">RANDORI</div>
    ...
    @if(!empty($needsRepair))
        <div style="background:#fcf3cf; border:1px solid #f1c40f; border-radius:16px; padding:16px 20px; margin-bottom:24px; display:flex; justify-content:space-between; align-items:center;">
            ...
            <button wire:click="repairBracket" class="btn-gen primary" style="background:#d4ac0d;"><i class="fas fa-wrench"></i> Perbaiki Bracket</button>
        </div>
    @endif
        ...
        <div class="bg-white rounded-2xl border border-rose-100 shadow-sm overflow-hidden">
            ...
            <div class="px-5 py-4 bg-gradient-to-r from-rose-600 to-rose-500 flex flex-col md:flex-row md:items-center justify-between gap-3">
                    ...
                    <div>
                        ...
                        <div class="text-lg font-black text-white uppercase tracking-tight leading-none">
                            ...
                            @if($activeMatch['bracket'] === 'gf')
                                Grand Final
                            @else
                                {{ strtoupper($activeMatch['bracket']) }} — Babak {{ $activeMatch['round'] + 1 }} / Match {{ $activeMatch['match'] + 1 }}
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div wire:ignore x-data="{
                    time: 0, running: false, countdown: 0,
                    lastTickSecond: -1, playedIntervals: new Set(),
                    interpolInterval: null, syncInterval: null,
                    formatTime() {
                    ...
                    async sync() {
                        ...
                        if (state.status === 'running') {
                            ...
                            this.running = false; this.countdown = 0;
                            this.time = state.elapsed_ms || 0;
                        }
                    ...
                    init() {
                        ...
                        this.interpolInterval = setInterval(() => {
                            if (this.running) {
                                ...
                                if (s === 90 && !this.playedIntervals.has(90)) { window.playBuzzer ? window.playBuzzer('/music/freesound_community-buzzerwav-14908.mp3') : null; this.playedIntervals.add(90); }
                                if (s === 120 && !this.playedIntervals.has(120)) { window.playBuzzer ? window.playBuzzer('/music/freesound_community-buzzerwav-14908.mp3') : null; this.playedIntervals.add(120); }
                                if (s > this.lastTickSecond) { window.playTimerTick ? window.playTimerTick(1000, 0.05) : null; this.lastTickSecond = s; }
                    ...
                    finish() {
                        ...
                        Swal.fire({
                            ...
                            html: '<p>Pertandingan akan ditandai <b>Selesai</b>.<br>Panggilan akan ditutup.</p>',
                            icon: 'warning', showCancelButton: true,
                            confirmButtonText: 'Ya, Selesai!', cancelButtonText: 'Batal', confirmButtonColor: '#2563eb',
                        }).then((r) => { if (r.isConfirmed) { $wire.finishMatch(); } else { $wire.startTimer(); } });
                    }
                }" class="flex items-center gap-3 bg-white/10 border border-white/20 px-4 py-2 rounded-xl">
                    <button @click="window.playBuzzer ? window.playBuzzer('/music/eritnhut1992-buzzer-or-wrong-answer-20582.mp3') : null" class="w-6 h-6 flex items-center justify-center bg-white/20 hover:bg-white/30 text-white rounded-full transition-colors" title="Test Suara">
                        <i class="fas fa-volume-up text-[10px]"></i>
                    </button>
                    <i class="fas fa-stopwatch text-amber-300"></i>
                    <div class="text-xl font-black font-mono tracking-wider min-w-[60px]" :class="countdown > 0 ? 'text-orange-300' : 'text-white'">
                        <span x-show="countdown > 0" x-text="formatCountdown()"></span>
                        <span x-show="countdown === 0" x-text="formatTime()">00:00</span>
                    </div>
                    <div class="flex items-center gap-1">
                        <button x-show="!running && countdown === 0" @click="start()" class="w-8 h-8 flex items-center justify-center bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg transition-colors shadow-sm" title="Start"><i class="fas fa-play text-xs"></i></button>
                        <button x-show="running" @click="pause()" class="w-8 h-8 flex items-center justify-center bg-amber-400 hover:bg-amber-500 text-white rounded-lg transition-colors shadow-sm" title="Pause"><i class="fas fa-pause text-xs"></i></button>
                        <button @click="stop()" class="w-8 h-8 flex items-center justify-center bg-white/20 hover:bg-white/30 text-white rounded-lg transition-colors shadow-sm" title="Reset"><i class="fas fa-stop text-xs"></i></button>
                        <button x-show="running || time > 0" @click="finish()" class="h-8 px-3 flex items-center justify-center bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-colors shadow-sm ml-1" title="Selesai">
            ...
            <div class="p-5 md:p-6 space-y-5">
                                    ...
                                    <tr class="hover:bg-slate-50/70 transition-colors">
                                        ...
                                        <td class="px-3 py-2.5 border-r border-slate-200">
                                            <div class="inline-flex rounded-lg overflow-hidden border border-slate-200">
                                                <button type="button" wire:click="updateScore('aka', '{{ $cat['key'] }}', -1)" class="px-2.5 py-1.5 bg-slate-50 hover:bg-rose-50 hover:text-rose-600 transition-colors text-slate-500"><i class="fas fa-minus text-[10px]"></i></button>
                                                <span class="px-3 py-1.5 bg-white min-w-[32px] font-black text-slate-800 border-x border-slate-200">{{ $scoringAka[$cat['key']] }}</span>
                                                <button type="button" wire:click="updateScore('aka', '{{ $cat['key'] }}', 1)" class="px-2.5 py-1.5 bg-slate-50 hover:bg-emerald-50 hover:text-emerald-600 transition-colors text-slate-500"><i class="fas fa-plus text-[10px]"></i></button>
                                            </div>
                                        </td>
                                        {{-- SHIRO --}}
                                        <td class="px-3 py-2.5 border-r border-slate-100 text-left">
                                        ...
                                        <td class="px-3 py-2.5">
                                            <div class="inline-flex rounded-lg overflow-hidden border border-slate-200">
                                                <button type="button" wire:click="updateScore('shiro', '{{ $cat['key'] }}', -1)" class="px-2.5 py-1.5 bg-slate-50 hover:bg-rose-50 hover:text-rose-600 transition-colors text-slate-500"><i class="fas fa-minus text-[10px]"></i></button>
                                                <span class="px-3 py-1.5 bg-white min-w-[32px] font-black text-slate-800 border-x border-slate-200">{{ $scoringShiro[$cat['key']] }}</span>
                                                <button type="button" wire:click="updateScore('shiro', '{{ $cat['key'] }}', 1)" class="px-2.5 py-1.5 bg-slate-50 hover:bg-emerald-50 hover:text-emerald-600 transition-colors text-slate-500"><i class="fas fa-plus text-[10px]"></i></button>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    ...
                    <button
                        wire:click="resetDetailedScoring"
                    ...
                    @foreach($matches as $matchIdx => $match)
                        ...
                        <div class="b-match {{ $isActive ? 'active' : '' }}">
                            <div class="b-slot {{ $isDone && $match['winner'] === 'athlete1' ? 'winner' : '' }}">
                                ...
                                @if($isDone && $match['winner'] === 'athlete1') <i class="fas fa-check-circle b-win-icon"></i> @endif
                            </div>
                            <div class="b-slot {{ $isDone && $match['winner'] === 'athlete2' ? 'winner' : '' }}">
                                ...
                                @if($isDone && $match['winner'] === 'athlete2') <i class="fas fa-check-circle b-win-icon"></i> @endif
                            </div>
                            @if(isset($match['athlete1']) && isset($match['athlete2']))
                                <div class="b-match-actions">
                                    @if(!$isActive && !$isDone)
                                        <button wire:click="callMatch('{{ $nodeKey }}', {{ $roundIdx }}, {{ $matchIdx }}, 'ub')" class="b-action-btn"><i class="fas fa-bullhorn"></i> Panggil</button>
                                    @elseif($isActive)
                                        <button wire:click="callMatch('{{ $nodeKey }}', {{ $roundIdx }}, {{ $matchIdx }}, 'ub')" class="b-action-btn"><i class="fas fa-redo"></i> Ulang</button>
                                        <button wire:click="openMatchModal('ub', {{ $roundIdx }}, {{ $matchIdx }})" class="b-action-btn" style="color:#2980b9;"><i class="fas fa-edit"></i> Skor</button>
                                        <button wire:click="dismissMatch()" class="b-action-btn" style="color:var(--red);"><i class="fas fa-times"></i> Tutup</button>
                                    @endif
                                </div>
                            @endif
                        </div>
                    ...
                    @foreach($matches as $matchIdx => $match)
                        ...
                        <div class="b-match {{ $isActive ? 'active' : '' }}">
                            ...
                            @if(isset($match['athlete1']) && isset($match['athlete2']))
                                <div class="b-match-actions">
                                    @if(!$isActive && !$isDone)
                                        <button wire:click="callMatch('{{ $nodeKey }}', {{ $lbRoundIdx }}, {{ $matchIdx }}, 'lb')" class="b-action-btn"><i class="fas fa-bullhorn"></i> Panggil</button>
                                    @elseif($isActive)
                                        <button wire:click="callMatch('{{ $nodeKey }}', {{ $lbRoundIdx }}, {{ $matchIdx }}, 'lb')" class="b-action-btn"><i class="fas fa-redo"></i> Ulang</button>
                                        <button wire:click="openMatchModal('lb', {{ $lbRoundIdx }}, {{ $matchIdx }})" class="b-action-btn" style="color:#2980b9;"><i class="fas fa-edit"></i> Skor</button>
                                        <button wire:click="dismissMatch()" class="b-action-btn" style="color:var(--red);"><i class="fas fa-times"></i> Tutup</button>
                                    @endif
                                </div>
                            @endif
        ...
        <div style="padding: 32px; display:flex; justify-content:center;">
            ...
            <div class="b-match {{ $gfActive ? 'active' : '' }}" style="width: 100%; max-width: 400px; transform: scale(1.1); box-shadow: 0 8px 24px rgba(243, 156, 18, 0.2);">
                <div class="b-slot {{ $gfDone && $grandFinal['winner'] === 'athlete1' ? 'winner' : '' }}" style="padding: 16px 20px;">
                    ...
                    @if($gfDone && $grandFinal['winner'] === 'athlete1') <i class="fas fa-trophy" style="color:#f1c40f; font-size:16px;"></i> @endif
                </div>
                ...
                <div class="b-slot {{ $gfDone && $grandFinal['winner'] === 'athlete2' ? 'winner' : '' }}" style="padding: 16px 20px;">
                    ...
                    @if($gfDone && $grandFinal['winner'] === 'athlete2') <i class="fas fa-trophy" style="color:#f1c40f; font-size:16px;"></i> @endif
                </div>
                @if(isset($grandFinal['athlete1']) && isset($grandFinal['athlete2']) && !$gfDone)
                    <div class="b-match-actions">
                        @if(!$gfActive && !$gfDone)
                            <button wire:click="callGrandFinal()" class="b-action-btn"><i class="fas fa-bullhorn"></i> Panggil GF</button>
                        @elseif($gfActive)
                            <button wire:click="callGrandFinal()" class="b-action-btn"><i class="fas fa-redo"></i> Ulang</button>
                            <button wire:click="openGrandFinalModal()" class="b-action-btn" style="color:#f39c12;"><i class="fas fa-edit"></i> Skor GF</button>
                            <button wire:click="dismissMatch()" class="b-action-btn" style="color:var(--red);"><i class="fas fa-times"></i> Tutup</button>
                        @endif
                    </div>
                @endif
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-xl overflow-hidden mt-12 mb-8">
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                @php
                    $medalConfig = [
                        1 => ['label' => 'Juara 1', 'icon' => '🥇', 'bg' => 'bg-amber-50', 'border' => 'border-amber-200', 'text' => 'text-amber-600'],
                        2 => ['label' => 'Juara 2', 'icon' => '🥈', 'bg' => 'bg-slate-50', 'border' => 'border-slate-200', 'text' => 'text-slate-600'],
                        3 => ['label' => 'Juara 3 Bersama', 'icon' => '🥉', 'bg' => 'bg-orange-50', 'border' => 'border-orange-200', 'text' => 'text-orange-600'],
                        4 => ['label' => 'Juara 3 Bersama', 'icon' => '🥉', 'bg' => 'bg-orange-50', 'border' => 'border-orange-200', 'text' => 'text-orange-600'],
                    ];
                @endphp
