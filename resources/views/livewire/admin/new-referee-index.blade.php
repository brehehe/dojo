<div>
    ...
    <style>
    /* ══════════════════════════════════════════════════════
       PAGE STYLES — Master Referee (Premium Layout)
    ══════════════════════════════════════════════════════ */
    ...
    .badge-wasit { background:rgba(212,168,67,0.1); color:#b8860b; }
    ...
    .modal-actions { 
        ...
        margin-top: 0; padding: 16px 24px 24px; 
    ...
    .section-title { font-family:'Cinzel',serif; font-size:13px; font-weight:700; color:var(--red); border-bottom:1px solid var(--paper2); padding-bottom:8px; margin-bottom:16px; margin-top:24px; }
    ...
    .modal-scroll-area::-webkit-scrollbar-track { background: var(--paper); border-radius: 10px; }
    .modal-scroll-area::-webkit-scrollbar-thumb { background: var(--paper2); border-radius: 10px; }
    .modal-scroll-area::-webkit-scrollbar-thumb:hover { background: var(--smoke); }
    </style>
    @endpush
    ...
    <div class="prem-page-a">
        ...
        <div class="page-hdr-a">
            <div>
                <h2>Master Wasit (Referee)</h2>
                <p>Kelola data wasit dan akun akses penjurian</p>
            </div>
            <button wire:click="showCreateModal" class="btn-prem-add">
                <i class="fa-solid fa-gavel"></i> Tambah Wasit
            </button>
        </div>
        ...
        <div class="stats-grid-a">
            <div class="stat-card-a gold">
                <div class="st-icon gold"><i class="fa-solid fa-gavel"></i></div>
                <div class="st-val">{{ $referees->total() }}</div>
                <div class="st-lbl">Total Wasit</div>
            </div>
        </div>
        {{-- TABLE --}}
        <div class="table-section-prem">
            ...
            <div class="search-field-prem">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari nama, email, atau no lisensi...">
            </div>
                    ...
                    <tbody>
                            ...
                            <td>
                                <div style="display:flex;align-items:center;gap:12px;">
                                    ...
                                    <div>
                                        ...
                                        <div style="font-size:12px;color:var(--smoke);">{{ $referee->user->email ?? '-' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div style="font-size:13px;font-weight:700;color:var(--ink);">{{ $referee->license_number ?: '-' }}</div>
                                <div style="margin-top:4px;">
                                    <span class="badge badge-wasit">{{ $referee->certification_level ?: 'Belum Ada' }}</span>
                                </div>
                            </td>
                            <td>
                                <div style="font-size:12px;font-weight:600;color:var(--ink);"><i class="fa-brands fa-whatsapp" style="color:#2ecc71;"></i> {{ $referee->phone ?: '-' }}</div>
                                <div style="font-size:11px;color:var(--smoke);margin-top:2px;">
                                    <i class="fa-solid fa-map-marker-alt"></i> {{ $referee->city ?: '-' }}, {{ $referee->province ?: '-' }}
                                </div>
                            </td>
                            <td>
                                <div style="display:flex;gap:5px;justify-content:flex-end;">
                                    <button wire:click="showEditModal({{ $referee->id }})" class="act-btn-a edit" title="Edit">
                                        <i class="fa-solid fa-pen"></i>
                                    </button>
                                    <button 
                                        {{ $referee->user_id === auth()->id() ? 'disabled' : '' }}
                        ...
                        <tr>
                            <td colspan="4" style="text-align:center;padding:40px 20px;">
                                <i class="fa-solid fa-user-tie" style="font-size:32px;color:var(--paper2);margin-bottom:12px;"></i>
                                <div style="font-family:'Cinzel',serif;font-weight:700;color:var(--ink);">Data Kosong</div>
                                <div style="font-size:12px;color:var(--smoke);">Belum ada data wasit.</div>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <x-modal wire:model.live="showingRefereeModal" maxWidth="3xl" :title="$refereeIdBeingEdited ? 'Edit Data Wasit' : 'Tambah Wasit Baru'">
                ...
                <div style="padding: 24px;">
                    ...
                    <div class="grid-3">
                        <div class="modal-field">
                            ...
                            <input wire:model="name" type="text" class="modal-input" placeholder="Nama wasit">
                    ...
                    <div class="grid-2">
                        <div class="modal-field">
                            ...
                            <input wire:model="certification_level" type="text" class="modal-input" placeholder="Contoh: Wasit Nasional A">
                            @error('certification_level') <span class="modal-err">{{ $message }}</span> @enderror
                        </div>
                        <div class="modal-field">
                            <label>Nomor Lisensi</label>
                            <input wire:model="license_number" type="text" class="modal-input" placeholder="No. Lisensi / Sertifikat">
                    ...
                    <div class="modal-field">
                        ...
                        <textarea wire:model="address" class="modal-textarea" placeholder="Alamat domisili wasit..."></textarea>
            ...
            <x-slot name="footer">
                ...
                <button type="submit" form="refereeForm" class="btn-save">
                    ...
                    <span wire:loading wire:target="saveReferee"><i class="fa-solid fa-circle-notch fa-spin"></i> Menyimpan...</span>
                </button>
            </x-slot>
        </x-modal>
    ...
    function confirmDelete(id, name) {
        Swal.fire({
            title: 'Hapus Wasit?',
            html: `Data wasit <b>${name}</b> dan akun loginnya akan dihapus permanen.`,
            icon: 'warning',
