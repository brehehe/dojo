<div>
    ...
    <style>
    /* ══════════════════════════════════════════════════════
       PAGE STYLES — Master Rundown (Premium Layout)
    ══════════════════════════════════════════════════════ */
    ...
    .badge-pertandingan { background:rgba(231,76,60,0.1); color:#e74c3c; }
    .badge-seremonial { background:rgba(52,152,219,0.1); color:#3498db; }
    ...
    <div class="prem-page-a">
        ...
        <div class="page-hdr-a">
            <div>
                <h2>Master Rundown</h2>
                <p>Kelola jadwal acara dan urutan kegiatan pertandingan</p>
            </div>
            <button wire:click="showCreateModal" class="btn-prem-add">
                <i class="fa-solid fa-calendar-alt"></i> Tambah Rundown
            </button>
        </div>
        ...
        <div class="stats-grid-a">
            <div class="stat-card-a gold">
                <div class="st-icon gold"><i class="fa-solid fa-calendar-check"></i></div>
                <div class="st-val">{{ $rundowns->total() }}</div>
                <div class="st-lbl">Total Agenda</div>
            </div>
        </div>
        {{-- TABLE --}}
        <div class="table-section-prem">
            <div class="table-toolbar-prem">
                <h3>Daftar Rundown Acara</h3>
            <div class="search-field-prem">
                <i class="fa-solid fa-magnifying-glass"></i>
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari agenda rundown...">
            </div>
            ...
            <div style="overflow-x:auto;">
                <table class="premium-table">
                    ...
                    <tbody>
                        @forelse($rundowns as $rd)
                        <tr>
                            <td>
                                <div class="group-name">{{ $rd->name }}</div>
                                <div style="margin-top:4px;">
                                    <span class="badge {{ $rd->type == 'pertandingan' ? 'badge-pertandingan' : 'badge-seremonial' }}">
                                        {{ ucfirst($rd->type) }}
                                    </span>
                                </div>
                            </td>
                            <td>
                                <div style="font-size:13px;font-weight:700;color:var(--ink);">
                                    <i class="fa-regular fa-calendar" style="color:var(--smoke);margin-right:4px;"></i> {{ \Carbon\Carbon::parse($rd->date)->format('d M Y') }}
                                </div>
                                <div style="font-size:12px;color:var(--red);font-weight:700;margin-top:2px;">
                                    <i class="fa-regular fa-clock" style="color:var(--smoke);margin-right:4px;"></i> {{ \Carbon\Carbon::parse($rd->date)->format('H:i') }} WIB
                                </div>
                            </td>
                            <td>
                                <div style="font-size:12px;color:var(--smoke);max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                                    {{ $rd->description ?: '-' }}
                                </div>
                            </td>
                            <td>
                                <div style="display:flex;gap:5px;justify-content:flex-end;">
                                    <button wire:click="showEditModal({{ $rd->id }})" class="act-btn-a edit" title="Edit">
                                        <i class="fa-solid fa-pen"></i>
                                    </button>
                                    <button onclick="confirmDelete({{ $rd->id }}, '{{ addslashes($rd->name) }}')" class="act-btn-a danger" title="Hapus">
                        ...
                        <tr>
                            <td colspan="4" style="text-align:center;padding:40px 20px;">
                                ...
                                <div style="font-size:12px;color:var(--smoke);">Belum ada jadwal rundown.</div>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
            ...
            <div style="padding:12px 16px;border-top:1px solid var(--paper2);">
                {{ $rundowns->links('livewire.admin.pagination') }}
            </div>
            @endif
        </div>
        ...
        <x-modal wire:model.live="showingRundownModal" maxWidth="md">
            <div style="padding:24px;">
                <div class="modal-hdr">
                    <div class="modal-title">{{ $rundownIdBeingEdited ? 'Edit Rundown' : 'Tambah Rundown' }}</div>
                </div>
                <form wire:submit="saveRundown">
                    <div class="modal-field">
                        <label>Nama Agenda / Kegiatan *</label>
                        <input wire:model="name" type="text" class="modal-input" placeholder="Contoh: Upacara Pembukaan">
                    ...
                    <div class="grid-2">
                        <div class="modal-field">
                            ...
                            <select wire:model="type" class="modal-select">
                                <option value="pertandingan">Pertandingan</option>
                                <option value="seremonial">Seremonial</option>
                            </select>
                            @error('type') <span class="modal-err">{{ $message }}</span> @enderror
                        </div>
                        <div class="modal-field">
                            <label>Waktu Pelaksanaan *</label>
                            <input wire:model="date" type="datetime-local" class="modal-input">
                            @error('date') <span class="modal-err">{{ $message }}</span> @enderror
                        </div>
                    </div>

                    <div class="modal-field">
                        <label>Keterangan Tambahan</label>
                        <textarea wire:model="description" class="modal-textarea" placeholder="Catatan opsional..."></textarea>
                        @error('description') <span class="modal-err">{{ $message }}</span> @enderror
                    </div>

                    <div class="modal-actions">
                        <button type="button" wire:click="$set('showingRundownModal', false)" class="btn-cancel">Batal</button>
                        <button type="submit" class="btn-save">
                            <span wire:loading.remove wire:target="saveRundown">Simpan Data</span>
                            <span wire:loading wire:target="saveRundown"><i class="fa-solid fa-circle-notch fa-spin"></i> Menyimpan...</span>
                        </button>
                    </div>
                </form>
            </div>
