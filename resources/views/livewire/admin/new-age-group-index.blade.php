<div>
    ...
    <style>
    /* ══════════════════════════════════════════════════════
       PAGE STYLES — Master Kelompok Umur (Premium Layout)
    ══════════════════════════════════════════════════════ */
    ...
    .group-price { font-size: 13px; font-weight: 600; color: #27ae60; background: rgba(39, 174, 96, 0.1); padding: 4px 10px; border-radius: 6px; display: inline-block; }
    ...
    <div class="prem-page-a">
        ...
        <div class="page-hdr-a">
            <div>
                <h2>Master Kelompok Umur</h2>
                <p>Kelola klasifikasi umur dan biaya pendaftaran untuk kategori pertandingan</p>
            </div>
            <button wire:click="showCreateModal" class="btn-prem-add">
                <i class="fa-solid fa-calendar-plus"></i> Tambah Kategori
            </button>
        </div>
        ...
        <div class="stats-grid-a">
            <div class="stat-card-a gold">
                <div class="st-icon gold"><i class="fa-solid fa-layer-group"></i></div>
                <div class="st-val">{{ $ageGroups->total() }}</div>
                <div class="st-lbl">Total Kelompok Umur</div>
            </div>
        </div>
        {{-- TABLE --}}
        <div class="table-section-prem">
            <div class="table-toolbar-prem">
                <h3>Daftar Kelompok Umur</h3>
            <div class="search-field-prem">
                <i class="fa-solid fa-magnifying-glass"></i>
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari kelompok umur...">
            </div>
                ...
                <table class="premium-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="group-price">Rp {{ number_format($group->price, 0, ',', '.') }}</span>
                            </td>
                            <td>
                                <div style="font-size:12px;font-weight:600;color:var(--ink);">{{ $group->created_at->format('d M Y') }}</div>
                                <div style="font-size:11px;color:var(--smoke);">{{ $group->created_at->diffForHumans() }}</div>
                            </td>
                            <td>
                                <div style="display:flex;gap:5px;justify-content:flex-end;">
                                    <button wire:click="showEditModal({{ $group->id }})" class="act-btn-a edit" title="Edit">
                                        <i class="fa-solid fa-pen"></i>
                                    </button>
                                    <button onclick="confirmDelete({{ $group->id }}, '{{ addslashes($group->name) }}')" class="act-btn-a danger" title="Hapus">
                        ...
                        <tr>
                            <td colspan="4" style="text-align:center;padding:40px 20px;">
                                ...
                                <div style="font-size:12px;color:var(--smoke);">Belum ada data kelompok umur yang ditambahkan.</div>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <x-modal wire:model.live="showingAgeGroupModal" maxWidth="md">
            <div style="padding:24px;">
                <div class="modal-hdr">
                    <div class="modal-title">{{ $ageGroupIdBeingEdited ? 'Edit Kelompok Umur' : 'Tambah Kelompok Umur' }}</div>
                </div>
                <form wire:submit="saveAgeGroup">
                    <div class="modal-field">
                        <label>Nama Kelompok Umur *</label>
                        <input wire:model="name" type="text" class="modal-input" placeholder="Contoh: Dewasa">
                        @error('name') <span class="modal-err">{{ $message }}</span> @enderror
                    </div>
                    <div class="modal-field">
                        <label>Biaya Pendaftaran (Rp) *</label>
                        <input wire:model="price" type="number" class="modal-input" placeholder="0">
                        @error('price') <span class="modal-err">{{ $message }}</span> @enderror
                    </div>
                    <div class="modal-actions">
                        <button type="button" wire:click="$set('showingAgeGroupModal', false)" class="btn-cancel">Batal</button>
                        <button type="submit" class="btn-save">
                            <span wire:loading.remove wire:target="saveAgeGroup">Simpan Data</span>
                            <span wire:loading wire:target="saveAgeGroup"><i class="fa-solid fa-circle-notch fa-spin"></i> Menyimpan...</span>
                        </button>
                    </div>
                </form>
            </div>
