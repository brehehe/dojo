<div>
    ...
    <div class="tm-page">
        {{-- FIXED RESET BUTTON --}}
        <div style="position: fixed; top: 65px; right: 30px; z-index: 90;">
                ...
                <div class="tm-stat-pill">
                    <div class="court-status">
                        ...
                        @if ($isActive)
                            <div style="display:flex; align-items:center; gap:8px;">
                                ...
                                <span
                                    style="width:10px;height:10px;border-radius:50%;background:#27ae60;animation:pulse 2s infinite;"></span>
                            </div>
                        @else
                        ...
                        @if ($isActive && $adMatch)
                            ...
                            @if ($adContingent)
                                <div class="court-match-contingent"><i class="fas fa-shield-alt"
                                        style="margin-right:4px;"></i>{{ $adContingent->name }}</div>
                            @endif
                            <div style="display:flex; gap:4px; flex-wrap:wrap; margin-top:6px;">
                                @if ($adPool)
                                    <span class="draw-badge" style="background:rgba(41,128,185,.1);color:#2980b9;">Pool
                                        {{ $adPool->name }}</span>
                                @endif
                                @if ($adSession)
                                    <span class="draw-badge"
                                        style="background:rgba(39,174,96,.1);color:#27ae60;">{{ $adSession->name }}</span>
                                @endif
                            </div>
                        @else
                            <div style="font-size:12px; color:var(--smoke); font-style:italic; padding-top:10px;">KOSONG
                                (Idle)</div>
                        @endif
                    </div>
                    ...
                    <div class="court-ref-list">
                        <div
                            ...
                            <span
                                style="font-size:9px; font-weight:700; color:var(--smoke); text-transform:uppercase;">Panel
                                Wasit</span>
                            @if ($courtCard->current_referees->count() > 0)
                                <button wire:confirm="Kosongkan panel wasit?"
                                    wire:click="resetActiveReferees({{ $courtCard->id }})"
                                    style="border:none;background:none;color:var(--red);font-size:9px;font-weight:700;cursor:pointer;text-transform:uppercase;">Reset</button>
                            @endif
                        </div>
                        @forelse($courtCard->current_referees as $sch)
                            <div class="court-ref-item">
                                <span
                                    style="width:16px;height:16px;border-radius:4px;background:var(--ink);color:#fff;display:flex;align-items:center;justify-content:center;font-size:8px;">{{ $sch->judge_index }}</span>
                                <span
                                    style="flex:1; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{{ $sch->referee?->name }}</span>
                            </div>
                        @empty
                    ...
                    <div class="court-links" style="grid-template-columns: repeat(2, 1fr);">
                        <a href="{{ route('admin.arbitrase.scoring.monitor', $courtCard->id) }}" target="_blank"
                            class="court-link"
                            style="background:var(--ink);color:#fff;border-color:var(--ink);">Panggilan</a>
                        <a href="{{ route('admin.arbitrase.scoring.monitor-hasil.court', $courtCard->id) }}"
                            target="_blank" class="court-link">Hasil</a>
                        <a href="{{ route('admin.arbitrase.scoring.monitor-timer.court', $courtCard->id) }}"
                            target="_blank" class="court-link">Timer</a>
                        <a href="{{ route('admin.arbitrase.scoring.monitor-rekapitulasi-hasil.court', $courtCard->id) }}"
                            target="_blank" class="court-link">Rekapitulasi</a>
                        <div style="display:flex; gap:4px; grid-column: span 2;">
                            <a href="{{ route('admin.arbitrase.scoring.monitor-referee.court', $courtCard->id) }}"
                                target="_blank" class="court-link" style="flex:1;">Wasit</a>
                            <button
                                ...
                                class="court-link" style="padding:4px 8px;cursor:pointer;"><i
                            ...
                            <tbody>
                                @forelse($drawings as $drawing)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="draw-badge {{ $isRandori ? 'randori' : 'embu' }} mb-1"
                                                style="margin-bottom:4px;display:inline-block;">{{ $drawing->draft_type }}</span>
        ...
        @if ($showRefereeModal)
            <div class="modal-overlay">
                <div class="modal-content">
                    <div class="modal-hdr">
                        <h3>Penugasan Wasit - {{ \App\Models\Court\Court::find($assigningCourtId)?->name ?? 'Lalu' }}
                        </h3>
                        <button wire:click="$set('showRefereeModal', false)" class="modal-close"><i
                    ...
                    <div class="modal-body">
                        <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:16px;">
                            <div>
                                <label
                                    style="font-size:10px; font-weight:700; color:var(--smoke); text-transform:uppercase; margin-bottom:4px; display:block;">Tanggal
                                    / Hari</label>
                                <select wire:model.live="assigningRundownId"
                            ...
                            <div>
                                <label
                                    style="font-size:10px; font-weight:700; color:var(--smoke); text-transform:uppercase; margin-bottom:4px; display:block;">Sesi
                                    / Shift</label>
                                <select wire:model.live="assigningSessionId"
                                    style="width:100%; padding:10px; border-radius:8px; border:1px solid var(--paper2); font-size:13px; outline:none;">
                        ...
                        <div style="margin-bottom:20px;">
                            <div
                                style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                                <span
                                    style="font-size:11px; font-weight:700; color:var(--smoke); text-transform:uppercase;">Wasit
                                    Terpilih</span>
                                <button wire:click="syncFromSchedule" class="btn-gen ghost"
                                    style="padding:4px 10px; font-size:9px;"><i class="fas fa-sync-alt"></i> Tarik
                                    dari Jadwal</button>
                            </div>
                            <div
                                    ...
                                    <div style="display:flex; flex-direction:column; gap:8px;">
                                        @foreach ($allReferees->whereIn('id', $selectedReferees)->sortBy(fn($r) => array_search((string) $r->id, $selectedReferees)) as $index => $ref)
                                            <div
                                                ...
                                                <span
                                                    style="width:24px; height:24px; border-radius:6px; background:var(--ink); color:#fff; display:flex; align-items:center; justify-content:center; font-size:10px; font-weight:700;">{{ array_search((string) $ref->id, $selectedReferees) + 1 }}</span>
                                                <div>
                                ...
                                @else
                                    <div style="text-align:center; padding:20px 0; color:var(--smoke);">
                                        <i class="fas fa-user-friends"
                                            style="font-size:24px; margin-bottom:8px; color:var(--paper2);"></i>
                                        <p style="font-size:12px; font-weight:600; margin:0;">Belum Ada Wasit Terpilih
                                        </p>
                                    </div>
                                @endif
                            </div>
                        ...
                        <div style="display:flex; gap:8px;">
                            <button wire:click="$set('showRefereeModal', false)" class="btn-gen ghost"
                                style="flex:1; padding:12px; justify-content:center; font-size:12px;">Batal</button>
                            <button wire:click="saveRefereeAssignment" class="btn-gen primary"
                                style="flex:2; padding:12px; justify-content:center; font-size:12px;"
                                {{ count($selectedReferees) !== 5 ? 'disabled' : '' }}>Ya, Tentukan Wasit</button>
                        </div>
                        @if (count($selectedReferees) > 0)
                            <button wire:confirm="Kosongkan wasit?" wire:click="resetCourtReferees"
                                class="btn-gen ghost"
                                style="width:100%; margin-top:8px; padding:10px; justify-content:center; color:var(--red); border-color:rgba(192,57,43,.2);">Kosongkan
                                Penugasan Wasit</button>
                        @endif
                    </div>
                </div>
            </div>
