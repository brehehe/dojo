    ...
    <div class="mb-12">
        <div class="flex items-center gap-4 mb-4">
            ...
            <div>
                ...
                <p class="text-slate-900 font-bold text-[15px] uppercase tracking-widest">Registration by Number (Tahap Pertama)</p>
            </div>
        </div>
    </div>
                                ...
                                <div class="p-6 bg-white rounded-3xl text-slate shadow-xl shadow-slate-900/20">
                                    ...
                                    <div class="space-y-4">
                                        <div class="flex items-center justify-between">
                                            <span class="text-[15px] font-bold text-slate-800 uppercase">Total Atlet</span>
                                            <span class="text-[15px] font-black">{{ $stats['totalAthletes'] }}</span>
                                        </div>
                                        <div class="flex items-center justify-between">
                                            <span class="text-[15px] font-bold text-slate-800 uppercase">Total Ofisial</span>
                                            <span class="text-[15px] font-black">{{ $stats['totalOfficials'] }}</span>
                                        </div>
                                        <div class="flex items-center justify-between pt-4 border-t border-white/10">
                                            <span class="text-[15px] font-bold text-orange-400 uppercase">Nomor Diikuti</span>
                                            <span class="text-[15px] font-black text-orange-400">{{ $stats['totalFollowed'] }}</span>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div>
                        @if($contingentId && $stats)
                            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                                <div>
                                    ...
                                    <p class="text-[15px] text-slate-800 font-bold uppercase tracking-widest">Daftar Kategori Lengkap</p>
                                </div>
                                    ...
                                    <table class="w-full text-left border-collapse border border-slate-200 rounded-xl overflow-hidden">
                                        <thead class="bg-slate-800 text-white">
                                            <tr class="bg-slate-900 text-white font-black uppercase text-[15px] tracking-widest">
                                                ...
                                                <th class="px-4 py-3 text-[15px] font-black uppercase tracking-widest border border-slate-700 whitespace-nowrap">Kategori</th>
                                                <th class="px-4 py-3 text-[15px] font-black uppercase tracking-widest border border-slate-700 whitespace-nowrap text-center">Ya</th>
                                                <th class="px-4 py-3 text-[15px] font-black uppercase tracking-widest border border-slate-700 whitespace-nowrap text-center">No</th>
                                            </tr>
                                        </thead>
                                            ...
                                            @foreach($genderMap as $genderKey => $genderLabel)
                                                @if(isset($categories[$genderKey]))
                                                    <tr class="bg-slate-100/50">
                                                        <td colspan="4" class="px-6 py-2.5 text-[15px] font-black text-slate-800 uppercase tracking-tight border-y border-slate-100 border-r border-slate-200">
                                                            {{ $genderLabel }}
                                                        </td>
                                                    </tr>
                                                    ...
                                                    @foreach($groupedByAge as $ageId => $matches)
                                                        <tr class="bg-slate-50/30">
                                                            <td colspan="4" class="px-8 py-2 text-[15px] font-black text-orange-600 uppercase tracking-widest italic font-serif border-r border-slate-200">
                                                        ...
                                                        @foreach($matches as $index => $match)
                                                            ...
                                                            <tr class="{{ $loop->even ? 'bg-slate-100' : 'bg-white' }} hover:bg-slate-50 transition-colors group">
                                                                <td class="px-6 py-3 text-center text-[15px] font-bold text-slate-800 font-mono border-r border-slate-200">{{ $index + 1 }}</td>
                                                                <td class="px-4 py-3 border-r border-slate-200">
                                                                    <p class="text-[15px] font-black text-black uppercase leading-none">{{ $match->name }}</p>
                                                                </td>
                                                                ...
                                                                <td class="px-4 py-3 text-center border-r border-slate-200">
                                                                    @if(!$isFollowed)
                                                                        <div class="w-6 h-6 rounded-lg bg-slate-100 text-slate-800 flex items-center justify-center mx-auto shadow-sm">
                        ...
                        @else
                            <div class="h-full flex flex-col items-center justify-center p-20 bg-slate-50 rounded-[3rem] border-4 border-dashed border-slate-100">
                                ...
                                <h3 class="text-[15px] font-black text-slate-300 uppercase tracking-widest mb-2 text-center">Menunggu Pilihan</h3>
                                <p class="text-[15px] text-slate-300 font-bold uppercase tracking-tight text-center max-w-xs">
