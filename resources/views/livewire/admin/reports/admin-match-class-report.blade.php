    ...
    <div class="mb-6 text-center lg:text-left">
        <div class="flex flex-col lg:flex-row items-center gap-4 mb-4">
            ...
            <div>
                ...
                <p class="text-slate-500 font-bold text-xs uppercase tracking-widest">Match Numbers & Class Composition</p>
            </div>
        </div>
    </div>
                        ...
                        @if($contingentId)
                            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                                <div>
                                    ...
                                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest italic pr-12">Menampilkan {{ count($matchGroups) }} nomor pertandingan yang diikuti.</p>
                                </div>
                            ...
                            <div class="space-y-12">
                                @forelse($matchGroups as $group)
                                    <div class="bg-slate-50 rounded-[2rem] border border-slate-100 p-8 hover:bg-white hover:shadow-2xl hover:shadow-slate-100 transition-all duration-500">
                                        <div class="flex items-center gap-4 mb-6 pb-6 border-b border-slate-200/50">
                                            <div class="w-10 h-10 bg-slate-900 text-white rounded-xl flex items-center justify-center font-black text-xs">
                                                <i class="fas fa-medal"></i>
                                            </div>
                                            <h4 class="text-sm font-black text-slate-800 uppercase tracking-tight">{{ $group['match_name'] }}</h4>
                                        </div>

                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                                            <!-- Participants -->
                                            <div>
                                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-4">Peserta</p>
                                                <div class="space-y-3">
                                                    @foreach($group['athletes'] as $athlete)
                                                        <div class="flex items-center gap-3">
                                                            <div class="w-8 h-8 rounded-lg bg-white border border-slate-100 flex items-center justify-center text-[10px] font-black text-orange-600 uppercase">{{ substr($athlete->athlete_name, 0, 1) }}</div>
                                                            <div>
                                                                <p class="text-xs font-black text-slate-700 uppercase leading-none mb-1">{{ $athlete->athlete_name }}</p>
                                                                <p class="text-[10px] text-slate-400 font-bold uppercase">{{ $athlete->tingkat ?: 'X Kyu' }}</p>
                                                            </div>
                                                        </div>
                                                    @endforeach
                                                </div>
                                            ...
                                            <div class="bg-white/50 rounded-2xl p-6 border border-slate-100">
                                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-4">Urutan Komposisi</p>
                                                <div class="space-y-2">
                                                    @forelse($group['techniques'] as $tIndex => $tName)
                                                        <div class="flex items-start gap-3">
                                                            <span class="text-[10px] font-black text-orange-500 mt-0.5">{{ $tIndex + 1 }}.</span>
                                                            <p class="text-xs font-bold text-slate-600 uppercase leading-none">{{ $tName }}</p>
                                                        </div>
                                                    @empty
                                                        <p class="text-[10px] text-slate-300 font-bold italic uppercase">Tidak ada komposisi teknik</p>
                                                    @endforelse
                                                </div>
                                            </div>
                                        </div>
                                ...
                                @empty
                                    <div class="h-[400px] flex flex-col items-center justify-center p-20 bg-slate-50 rounded-[3rem] border-4 border-dashed border-slate-100 italic text-slate-300">
                                        ...
                                        <p class="text-xs font-black uppercase tracking-widest">Data tidak ditemukan</p>
                                    </div>
                                @endforelse
                            </div>
                        @else
                            <div class="h-[600px] flex flex-col items-center justify-center p-20 bg-slate-50 rounded-[3.5rem] border-4 border-dashed border-slate-100">
                                ...
                                <p class="text-[11px] text-slate-300 font-bold uppercase tracking-tight text-center max-w-xs px-10 leading-relaxed">
