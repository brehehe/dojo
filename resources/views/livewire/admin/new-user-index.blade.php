<div>
    ...
    <style>
    /* ══════════════════════════════════════════════════════
       PAGE STYLES — Master User (Premium Layout)
    ══════════════════════════════════════════════════════ */
    ...
    .badge-admin { background:rgba(192,57,43,0.1); color:#c0392b; }
    .badge-wasit { background:rgba(212,168,67,0.1); color:#b8860b; }
    .badge-kontingen { background:rgba(52,152,219,0.1); color:#3498db; }
    .badge-default { background:rgba(149,165,166,0.1); color:#7f8c8d; }
    ...
    .act-btn-a:disabled { opacity: 0.5; cursor: not-allowed; border-color: var(--paper2); color: var(--smoke); background: var(--paper); }
    ...
    <div class="prem-page-a">
        ...
        <div class="page-hdr-a">
            <div>
                ...
                <p>Kelola data akun dan hak akses pengguna sistem</p>
            </div>
        ...
        <div class="stats-grid-a">
            <div class="stat-card-a gold">
                <div class="st-icon gold"><i class="fa-solid fa-users"></i></div>
                <div class="st-val">{{ $users->total() }}</div>
                <div class="st-lbl">Total Akun</div>
            </div>
        </div>
        {{-- TABLE --}}
                ...
                <table class="premium-table">
                    ...
                    <tbody>
                        ...
                        @php
                            ...
                            if(str_contains(strtolower($roleName), 'admin')) $badgeClass = 'badge-admin';
                            elseif(str_contains(strtolower($roleName), 'wasit')) $badgeClass = 'badge-wasit';
                            elseif(str_contains(strtolower($roleName), 'kontingen')) $badgeClass = 'badge-kontingen';
                        @endphp
                        <tr>
                            <td>
                                <div style="display:flex;align-items:center;gap:12px;">
                                    ...
                                    <div>
                                        ...
                                        <div style="font-size:12px;color:var(--smoke);">{{ $user->email }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                <div style="font-size:12px;font-weight:600;color:var(--ink);">{{ $user->created_at->format('d M Y') }}</div>
                                <div style="font-size:11px;color:var(--smoke);">{{ $user->created_at->format('H:i') }} WIB</div>
                            </td>
                            <td>
                                <div style="display:flex;gap:5px;justify-content:flex-end;">
                                    <button wire:click="showEditModal({{ $user->id }})" class="act-btn-a edit" title="Edit">
                                        <i class="fa-solid fa-pen"></i>
                                    </button>
                                    <button 
                                        {{ $user->id === auth()->id() ? 'disabled' : '' }}
                                        onclick="confirmDelete({{ $user->id }}, '{{ addslashes($user->name) }}')" 
                                        class="act-btn-a danger" 
                        ...
                        <tr>
                            <td colspan="4" style="text-align:center;padding:40px 20px;">
                                <i class="fa-solid fa-user-xmark" style="font-size:32px;color:var(--paper2);margin-bottom:12px;"></i>
                                <div style="font-family:'Cinzel',serif;font-weight:700;color:var(--ink);">Data Kosong</div>
                                <div style="font-size:12px;color:var(--smoke);">Belum ada data pengguna.</div>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        {{-- MODAL --}}
        @if($showingUserModal)
        <x-modal wire:model.live="showingUserModal" maxWidth="md">
            <div style="padding:24px;">
                <div class="modal-hdr">
                    <div class="modal-title">{{ $userIdBeingEdited ? 'Edit Pengguna' : 'Tambah Pengguna' }}</div>
                </div>
                <form wire:submit="saveUser">
                    <div class="modal-field">
                        <label>Nama Lengkap *</label>
                        <input wire:model="name" type="text" class="modal-input" placeholder="Nama pengguna">
                        @error('name') <span class="modal-err">{{ $message }}</span> @enderror
                    </div>
                    
                    <div class="modal-field">
                        <label>Alamat Email *</label>
                        <input wire:model="email" type="email" class="modal-input" placeholder="email@contoh.com">
                    ...
                    <div class="grid-2">
                        <div class="modal-field">
                            ...
                            <select wire:model="selectedRole" class="modal-select">
                                <option value="">Pilih Role...</option>
                                @foreach($roles as $role)
                                    <option value="{{ $role->name }}">{{ $role->name }}</option>
                                @endforeach
                            </select>
                            @error('selectedRole') <span class="modal-err">{{ $message }}</span> @enderror
                        </div>
                        <div class="modal-field">
                            <label>Password {{ $userIdBeingEdited ? '(Kosongkan jika tidak diubah)' : '*' }}</label>
                            <input wire:model="password" type="password" class="modal-input" placeholder="Minimal 8 karakter">
                            @error('password') <span class="modal-err">{{ $message }}</span> @enderror
                        </div>
                    </div>

                    <div class="modal-actions">
                        <button type="button" wire:click="$set('showingUserModal', false)" class="btn-cancel">Batal</button>
                        <button type="submit" class="btn-save">
                            <span wire:loading.remove wire:target="saveUser">Simpan Data</span>
                            <span wire:loading wire:target="saveUser"><i class="fa-solid fa-circle-notch fa-spin"></i> Menyimpan...</span>
                        </button>
                    </div>
                </form>
            </div>
    ...
    function confirmDelete(id, name) {
        Swal.fire({
            ...
            html: `Akun <b>${name}</b> akan dihapus permanen.`,
            icon: 'warning',
