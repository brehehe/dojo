<div>
    ...
    <style>
    /* ══════════════════════════════════════════════════════
       PAGE STYLES — Master Contingent (Premium Layout)
    ══════════════════════════════════════════════════════ */
    ...
    .cnt-logo { width:40px; height:40px; border-radius:8px; object-fit:cover; border:1px solid var(--paper2); }
    ...
    <div class="prem-page-a">
        ...
        <div class="stats-grid-a">
            <div class="stat-card-a gold">
                <div class="st-icon gold"><i class="fa-solid fa-flag"></i></div>
                <div class="st-val">{{ $contingents->total() }}</div>
                <div class="st-lbl">Total Kontingen</div>
            </div>
        </div>
        {{-- TABLE --}}
        <div class="table-section-prem">
            ...
            <div class="search-field-prem">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari nama kontingen atau manajer...">
            </div>
            <select wire:model.live="statusFilter" class="perpage-select-prem">
            ...
            <div style="overflow-x:auto;">
                <table class="premium-table">
                    ...
                    <tbody>
                        @forelse($contingents as $ctg)
                        <tr>
                            <td>
                                <div style="display:flex;align-items:center;gap:12px;">
                                    @if($ctg->logo)
                                        <img src="{{ Storage::url($ctg->logo) }}" alt="Logo" class="cnt-logo">
                                    @else
                                        <div style="width:40px;height:40px;border-radius:8px;background:var(--paper);display:flex;align-items:center;justify-content:center;color:var(--smoke);border:1px solid var(--paper2);">
                                            <i class="fa-solid fa-flag"></i>
                                        </div>
                                    @endif
                                    <div>
                                        <div class="group-name">{{ $ctg->name }}</div>
                                        <div style="font-size:11px;color:var(--smoke);">Didaftarkan: {{ $ctg->created_at->format('d M Y') }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div style="font-size:13px;font-weight:700;color:var(--ink);">{{ $ctg->kab_kota }}</div>
                                <div style="font-size:12px;color:var(--smoke);">{{ $ctg->province }}</div>
                            </td>
                            <td>
                                <div style="font-size:13px;font-weight:600;color:var(--ink);">{{ $ctg->leader_name }}</div>
                                <div style="font-size:11px;color:var(--smoke);"><i class="fa-brands fa-whatsapp" style="color:#2ecc71;"></i> {{ $ctg->phone ?: '-' }}</div>
                            </td>
                            <td>
                                <div style="display:flex;gap:12px;">
                                    <div style="text-align:center;">
                                        <div style="font-family:'Cinzel',serif;font-size:16px;font-weight:700;color:var(--red);">{{ $ctg->athletes_count }}</div>
                                        <div style="font-size:10px;text-transform:uppercase;color:var(--smoke);font-weight:700;">Atlet</div>
                                    </div>
                                    <div style="text-align:center;">
                                        <div style="font-family:'Cinzel',serif;font-size:16px;font-weight:700;color:var(--ink);">{{ $ctg->officials_count }}</div>
                                        <div style="font-size:10px;text-transform:uppercase;color:var(--smoke);font-weight:700;">Official</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                        ...
                        <tr>
                            <td colspan="5" style="text-align:center;padding:40px 20px;">
                                <i class="fa-solid fa-flag-checkered" style="font-size:32px;color:var(--paper2);margin-bottom:12px;"></i>
                                <div style="font-family:'Cinzel',serif;font-weight:700;color:var(--ink);">Data Kosong</div>
                                <div style="font-size:12px;color:var(--smoke);">Belum ada data kontingen yang didaftarkan.</div>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
