<div>
    ...
    <style>
    /* ══════════════════════════════════════════════════════
       PAGE STYLES — Master Court (Premium Layout)
    ══════════════════════════════════════════════════════ */
    ...
    <div class="prem-page-a">
        ...
        <div class="page-hdr-a">
            <div>
                <h2>Master Lapangan (Court)</h2>
                <p>Kelola data lapangan untuk jadwal pertandingan</p>
            </div>
            <button wire:click="showCreateModal" class="btn-prem-add">
                <i class="fa-solid fa-layer-group"></i> Tambah Lapangan
            </button>
        </div>
        ...
        <div class="stats-grid-a">
            <div class="stat-card-a gold">
                ...
                <div class="st-val">{{ $courts->total() }}</div>
                <div class="st-lbl">Total Lapangan</div>
            </div>
        </div>
        {{-- TABLE --}}
        <div class="table-section-prem">
            <div class="table-toolbar-prem">
                <h3>Daftar Lapangan (Court)</h3>
            <div class="search-field-prem">
                <i class="fa-solid fa-magnifying-glass"></i>
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari nama lapangan...">
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <div style="display:flex;align-items:center;gap:12px;">
                                    ...
                                    <span class="group-name">{{ $court->name }}</span>
                                </div>
                            </td>
                            <td>
                                <div style="font-size:12px;font-weight:600;color:var(--ink);">{{ $court->created_at->format('d M Y') }}</div>
                                <div style="font-size:11px;color:var(--smoke);">{{ $court->created_at->diffForHumans() }}</div>
                            </td>
                            <td>
                                <div style="display:flex;gap:5px;justify-content:flex-end;">
                                    <button wire:click="showEditModal({{ $court->id }})" class="act-btn-a edit" title="Edit">
                                        <i class="fa-solid fa-pen"></i>
                                    </button>
                                    <button onclick="confirmDelete({{ $court->id }}, '{{ addslashes($court->name) }}')" class="act-btn-a danger" title="Hapus">
                        ...
                        <tr>
                            <td colspan="3" style="text-align:center;padding:40px 20px;">
                                ...
                                <div style="font-size:12px;color:var(--smoke);">Belum ada data lapangan yang ditambahkan.</div>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <x-modal wire:model.live="showingCourtModal" maxWidth="md">
            <div style="padding:24px;">
                <div class="modal-hdr">
                    <div class="modal-title">{{ $courtIdBeingEdited ? 'Edit Lapangan' : 'Tambah Lapangan' }}</div>
                </div>
                <form wire:submit="saveCourt">
                    <div class="modal-field">
                        <label>Nama Lapangan *</label>
                        <input wire:model="name" type="text" class="modal-input" placeholder="Contoh: Court A">
                        @error('name') <span class="modal-err">{{ $message }}</span> @enderror
                    </div>
                    <div class="modal-actions">
                        <button type="button" wire:click="$set('showingCourtModal', false)" class="btn-cancel">Batal</button>
                        <button type="submit" class="btn-save">
                            <span wire:loading.remove wire:target="saveCourt">Simpan Data</span>
                            <span wire:loading wire:target="saveCourt"><i class="fa-solid fa-circle-notch fa-spin"></i> Menyimpan...</span>
                        </button>
                    </div>
                </form>
            </div>
