...
{
    /**
     * Build the complete referee scoring dashboard state for a given user.
     *
     * @return array<string, mixed>
     */
    public function getRefereeScoringState(User $user): array
    {
        ...
        if ($isTabletMode) {
            ...
            if ($assignedCourt && $assignedCourt->active_match_id) {
                ...
                if (! $activeDrawing) {
                    $activeDrawing = DrawingMatchNumber::with('pool')
                        ->where('match_number_id', $assignedCourt->active_match_id)
    ...
    /**
     * Save referee scoring details (draft auto-save).
     *
     * @param  array<string, mixed>  $embuItems
     * @return array{success: bool, message?: string}
     */
    public function saveScore(User $user, array $embuItems, string $notes = ''): array
    {
        ...
        if (! $stateResult || ! $stateResult['referee'] || ! $stateResult['activeMatch'] || ! $stateResult['judgeIndex']) {
            return ['success' => false, 'message' => 'Invalid assignment or active match.'];
        }
        ...
        if ($activeMatch->draft_type === 'embu') {
            ...
            return ['success' => false, 'message' => 'Kategori Randori tidak dinilai di panel ini.'];
        }

        if (! $id) {
            return ['success' => false, 'message' => 'No active contestant.'];
        }
        ...
        $totalScore = array_sum($embuItems);
    ...
    /**
     * Submit referee scores and sign off.
     *
     * @param  array<string, mixed>  $embuItems
     * @return array{success: bool, message?: string}
     */
    public function submitScore(User $user, string $signature, array $embuItems = [], string $notes = ''): array
    {
        $stateResult = $this->resolveRefereeState($user);
        if (! $stateResult || ! $stateResult['referee'] || ! $stateResult['activeMatch'] || ! $stateResult['judgeIndex']) {
            return ['success' => false, 'message' => 'Invalid assignment or active match.'];
        ...
        if (! $id) {
            return ['success' => false, 'message' => 'Belum ada peserta yang dipanggil.'];
        }
     ...
     *
     * @return array{referee: ?Referee, activeMatch: ?MatchNumber, assignedCourt: ?Court, judgeIndex: ?int}|null
     */
    public function resolveRefereeState(User $user): ?array
    {
        $isTabletMode = ! empty($user->judge_index) && ! empty($user->court_id);
        $referee = null;
        $activeMatch = null;
    ...
    public function getSpecificMatchId($activeMatch, ?Court $assignedCourt): int
    {
        $matchNumberIds = [$activeMatch->id];
        if ($activeMatch->mergeDetail) {
            $matchNumberIds = MatchNumberMergeDetail::where('match_number_merge_id', $activeMatch->mergeDetail->match_number_merge_id)
    ...
    public function getJudgeLabel(int $index): string
    {
        return match ($index) {
